...
main = do
    result <- flip fmap BS8.getContents $ runInput $ do
        n <- readInt
        as <- traverse (`V.replicateM`readInt) [2*n-1,2*n-2 .. 1]
        return . show $ solv n as
    putStrLn result

solv :: Int -> V.Vector (V.Vector Int) -> Int
solv n as = runST $ do
    tab <- V.thaw $ V.replicate (2*n) False
    go tab [] 0
    where
        go :: MV.MVector s Bool -> [(Int, Int)] -> Int -> ST s Int
        go tab pairs mx = do
            mf <- firstFalse tab 0
            case mf of
                Just f -> do
                    withFlag tab f (go2 tab f (f+1) pairs mx)
                Nothing ->
                    return $ foldl (\s (i, j) -> s `xor` (as ! i ! (j-i-1))) 0 pairs

        go2 :: MV.MVector s Bool -> Int -> Int -> [(Int, Int)] -> Int -> ST s Int
        go2 tab i j pairs mx
            | j == (n*2) = return mx
            | otherwise = filterM ((not<$>) . MV.read tab) [j..2*n-1] >>= 
                    foldM (\mx' k -> max mx' <$> withFlag tab k (go tab ((i, k):pairs) mx)) 0 

        firstFalse arr i
            | i == (n*2) = return Nothing
            | otherwise = MV.read arr i >>= \case
                False -> return (Just i)
                True -> firstFalse arr (i+1)

        withFlag tab i m = MV.write tab i True >> m <* MV.write tab i False
