...
main = runInput $ do
    ...
    k <- readInt
    as <- readInts n
    liftIO . print $ solv n k as

solv n k as = runST $ do
    cnt <- newSTRef 1
    runHeapDP HeapAsc (go cnt) (toList as) 0
    where
    go :: STRef s Int -> Int -> HeapDP Int Int s (ST s) (V.Vector Int)
    go cnt i = do
        whenM ((==k) <$> liftPrim (readSTRef cnt)) $ quit i
        liftPrim $ modifySTRef cnt succ
        return $ V.map (+i) as
...
type HeapDP s m a = StateT (Set s) m a
runHeapDP :: (Monad m, Ord s, Foldable f) => (s -> Either r (f s)) -> [s] -> r -> m r
runHeapDP dp initial r = evalStateT go (Set.fromList initial) where
    go = do
        heap <- State.get
        if Set.null heap then return r else do
            let (a, heap') = Set.deleteFindMin heap
            case dp a of
                Right nexts -> do
                    traverse_ (State.modify . Set.insert) nexts >> go
                Left r -> return r
