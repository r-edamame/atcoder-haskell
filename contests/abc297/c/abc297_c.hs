...
main = do
    result <- flip fmap BS8.getContents $ runInput $ do
        h <- readInt
        w <- readInt
        ss <- V.replicateM h $ V.replicateM w readChar
        return . unlines . toList . V.map toList $ solv h w ss
    putStr result

solv h w ss = V.map f ss where
    f s = runST $ do
        v <- V.thaw $ s
        forM_ @[] [0..V.length s-2] $ \i ->
            whenM ((&&) <$> ((=='T') <$> MV.read v i) <*> ((=='T') <$> MV.read v (i+1))) $ do
                MV.write v i 'P'
                MV.write v (i+1) 'C'
        V.freeze v
