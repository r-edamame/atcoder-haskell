...
main = runInput $ do
    n <- readInt
    ps <- readTs n
    liftIO $ forM_ (solv n ps) $ print . succ

solv :: Int -> V.Vector (Int, Int) -> VI
solv n ps = V.map V.maxIndex . V.map (\a -> V.map (dissqr a) ps) $ ps where
    dissqr :: (Int, Int) -> (Int, Int) -> Int
    dissqr (x0, y0) (x1, y1) = (x0 - x1).pow 2 + (y0 - y1).pow 2
...
instance HasField "asDouble" Int Double where
    getField = fromRational . fromIntegral

instance HasField "pow" Int (Int -> Int) where
    getField = (^)
-- Data
---- Heap
