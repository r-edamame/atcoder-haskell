...
import           Control.Monad (replicateM, forM_, when, foldM, filterM, forM)
...
main = runInput $ do
    ...
    mat <- readCharMat w h
    n <- readInt
    ms <- V.replicateM n $ (,) <$> (flip (,) <$> readInt1 <*> readInt1) <*> readInt
    liftIO $ putStrLn $ yesno $ solv w h mat n ms

{- 考察
 - スタートからそれぞれの薬の場所、
 - またそれぞれの薬の場所からほかの薬の場所までの最短経路をそれぞれ求め、
 - スタートでは初期のエネルギー・それ以外ではその地点の薬を使った際のエネルギーで到達できないものは除外する
 - それぞれの薬の地点で薬を使った場合エネルギーは決まった値になるため、
 - 経路によるエネルギーの残高を気にせず独立して計算できる
 - またそれぞれの点からゴールへの最短経路を計算する
 - これにより、スタート・薬の位置・ゴールについて到達可能かどうかによる有向グラフが完成するため、
 - あとはその中で探索をしてゴールにたどり着けるかどうかを判定する
 -}

solv :: Int -> Int -> Mat Char -> Int -> V.Vector ((Int, Int), Int) -> Bool
solv w h mat n ms = isJust $ mspi >>= searchFind (\p -> if p == n then Left gp else Right (conns.at p)) . Seq.singleton where
    sp = let y = fromJust $ V.findIndex (V.elem 'S') mat
        in (fromJust $ V.findIndex (=='S') (mat.at y), y)
    gp = let y = fromJust $ V.findIndex (V.elem 'T') mat
        in (fromJust $ V.findIndex (=='T') (mat.at y), y)
    mspi = V.findIndex ((==sp) . fst) ms
    ps = ms V.++ [(gp, 0)]
    conns = V.fromList $ map (searchShortest w h mat ps) [0..n-1]

-- ある点から対応した体力において到達可能な点の一覧
searchShortest :: Int -> Int -> Mat Char -> V.Vector ((Int, Int), Int) -> Int -> [Int]
searchShortest w h mat ps from =
    let passed = search f fst ([((fx, fy), 0)] :: Seq ((Int, Int), Int))
    in filter (flip HSet.member passed . fst . ps.at) [0..ps.len - 1]
    where
        ((fx, fy), fe) = ps.at from
...
printV tobs v = liftIO $ do
    print v.len
    BS8.putStrLn . BS8.unlines . toList $ V.map tobs v
...
newtype HalfOpen = HalfOpen { getHalfOpen :: (Int, Int) } deriving (Eq)
instance Range HalfOpen where
    outOf (HalfOpen (s, t)) i = i < s || t <= i
...
instance HasField "x" (a, a) a where
    getField = fst
instance HasField "y" (a, a) a where
    getField = snd
...
quitableFold :: (s -> Either a s) -> s -> Either a s
quitableFold step = internal where
    internal s = step s >>= internal
