...
main = runInput $ do
    n <- readInt
    s <- V.fromList . map (fromEnum . (=='1')) . BS8.unpack <$> readWord
    cs <- readInts n
    liftIO $ print $ solv n s cs

solv :: Int -> VI -> VI -> Int
solv n s cs
solv n s cs = runST $ do
    -- dp[i][j][k] = 前からi桁目についてのコストの最小値(初期値がj)(k=1 -> 重複が存在している)
    dp <- V.replicateM (n+1) (V.replicateM 2 (V.thaw (V.replicate 2 (maxBound :: Int))))
    ((dp.at 0).at 0).set 0 0
    ((dp.at 0).at 1).set 0 0
    forM_ @[] []
    return 0
