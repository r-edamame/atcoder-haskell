...
main = runInput $ do
    s <- readWord
    liftIO . putStrLn . yesno $ solv ["erase", "eraser", "dream", "dreamer"] s

solv :: [ByteString] -> ByteString -> Bool
solv ws s = runST $ do
    dp <- V.thaw $ V.replicate (s.len + 1) False
    dp.set 0 True
    forM_ @[] [0..s.len-1] $ \i ->
        whenM (dp.at i) $
            forM_ ws $ \w ->
                let e = i + w.len
                in when (e <= s.len && s.substr i e == w) $
                    dp.set e True
    dp.at s.len
...
instance HasField "substr" ByteString (Int -> Int -> ByteString) where
    getField bs from to = BS.drop from $ BS.take to bs
instance HasField "read" (STRef s a) (ST s a) where
    getField = readSTRef
instance HasField "write" (STRef s a) (a -> ST s ()) where
    getField = writeSTRef
