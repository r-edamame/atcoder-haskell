...
import           Control.Monad.Trans.Cont (ContT(ContT, runContT), Cont, cont, runCont, evalContT)
...
main = do
    result <- flip fmap BS8.getContents $ runInput $ do
        ...
        ab <- readTs1 m
        cd <- readTs1 m
        return . yesno $ solv n ab cd
    putStrLn result

solv :: Int -> V.Vector (Int, Int) -> V.Vector (Int, Int) -> Bool
solv n ab cd = runST $ do
    arr <- V.thaw [0..n-1]
    evalContT $ runReaderT go arr
    where
    g1 = udg n ab
    g2 = udg n cd
    go :: ReaderT (MV.MVector s Int) (ContT Bool (ST s)) Bool
    go = do
        v <- Reader.ask
        same <- fmap and $ forM @[] [0..n-1] $ \i -> do
            j <- debug "j" <$> readVr (debug "i" i)
            fmap and $ forM (g1!i) $ \l -> readVr l >>= \k -> return $ isJust (V.find (==k) (g2!j))
        when same $ lift $ quit True
        whenM (not <$> MV.nextPermutation v) $ lift (quit False)
        go
