...
import           Control.Monad.State.Strict (StateT (StateT, runStateT), evalStateT)
import qualified Control.Monad.State.Class as State
import           Control.Monad.Trans.Class (lift)
import           Control.Monad.Trans.Cont (ContT(runContT), Cont, cont, runCont, evalContT)
...
main = do
    result <- flip fmap BS8.getContents $ runInput $ do
        ...
        w <- readInt
        mat <- readCharMat w h
        return . show $ solv w h mat
    putStrLn result

solv :: Int -> Int -> Mat Char -> Int
solv w h (mat, ix) = runST $ do
    vs <- V.replicateM h $ V.thaw (V.replicate w 0)
    runBfs (go vs) (Seq.singleton (0,0))
    maximum <$> traverse (foldMV max 0) vs
    where
    f v x y = if x < 0 || w <= x || y < 0 || h <= y then return 0 else MV.read (v!y) x
    go vs (x, y) = do
        mx <- max <$> f vs (x-1) y <*> f vs x (y-1)
        MV.write (vs!y) x (mx+1)
        when (y<h-1 && mat ! ix x (y+1) == '.') $ queue (x, y+1)
        when (x<w-1 && mat ! ix (x+1) y == '.') $ queue (x+1, y)
...
---- BFS
type BFS q m a = StateT (Seq q) m a
runBfs :: (Monad m, Eq q, Hashable q) => (q -> BFS q m ()) -> Seq q -> m ()
runBfs bfs = evalStateT (go HSet.empty) where
    -- go :: Monad m => (q -> BFS a q m a) -> a -> BFS a q m a
    go used = do
        q <- State.get
        case q of
            Seq.Empty -> return ()
            a :<| q'
                | HSet.member a used -> State.put q' >> go used
                | otherwise -> do
                    State.put q'
                    bfs a
                    go (HSet.insert a used)
queue :: Monad m => q -> BFS q m ()
