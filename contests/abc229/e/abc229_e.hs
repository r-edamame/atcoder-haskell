...
main = runInput $ do
    ...
    m <- readInt
    es <- readTs1 m
    liftIO . putStr . unlines . map show $ solv n m es

solv n m es = runST $ do
    uf <- newUF n
    go uf (n-1) [0]
    where
    g = udg' n es
    go _ 0 !anss = return anss
    go uf i !anss = do
        forM_ (g!i) $ \t ->
            uniteUF i t uf
        siz <- lenUF uf
        go uf (i-1) ((siz-i):anss)

udg' :: Int -> V.Vector (Int, Int) -> V.Vector (V.Vector Int)
udg' n v = V.accumulate (flip V.cons) (V.replicate n V.empty) to where
    to :: V.Vector (Int, Int)
    to = v >>= \(x, y) -> [(min x y, max x y)]
...
---- UnionFind
newtype UnionFind s = UnionFind (MV.MVector s Int, MV.MVector s Int, MV.MVector s Int, STRef s Int)
...
newUF n = do
    ...
    len <- newSTRef n
    return $ UnionFind (par, rank, siz, len)
{-# INLINE newUF #-}
rootUF :: Int -> UnionFind s -> ST s Int
rootUF x uf@(UnionFind (par, _, _, _))= do
    p <- MV.read par x
    if p == x
        then
            return x
...
uniteUF x y uf@(UnionFind (par, rank, siz, len)) = do
...
sizeUF x uf@(UnionFind(_, _, siz, _)) = MV.read siz x
{-# INLINE sizeUF #-}
lenUF :: UnionFind s -> ST s Int
lenUF (UnionFind (_, _, _, len)) = readSTRef len
{-# INLINE lenUF #-}
---- undirected graph
udg :: Int -> V.Vector (Int, Int) -> V.Vector (V.Vector Int)
udg n v = V.accumulate (flip V.cons) (V.replicate n V.empty) to where
