...
main :: IO ()
main = runInput $ do
    a <- BS8.unpack <$> readWord
    b <- BS8.unpack <$> readWord
    let ml = max (length a) (length b)
    let a' = map digitToInt $ padL ml '0' a
    let b' = map digitToInt $ padL ml '0' b
    let f b = if b then "Easy" else "Hard"
    liftIO . putStrLn . f $ all ((<10).uncurry (+)) (zip a' b')

padL :: Int -> a -> [a] -> [a]
padL n a xs = replicate (n-length xs) a ++ xs
