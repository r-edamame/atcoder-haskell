...
main = runInput $ do
    ...
    w <- readInt
    cs <- readTs n
    liftIO . print $ solv n w cs

solv n w cs'' = go 0 0 cs' where
    cs' = V.modify (VH.sortBy (flip compare)) cs''
    go score q cs = case unconsV cs of
        Nothing -> score
        Just ((a, b), r)
            | b <= w-q -> go (score + a*b) (q+b) r
            | otherwise -> score + (a * (w-q))
