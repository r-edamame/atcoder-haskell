...
solv :: ByteString -> Bool
solv s = all (==2) $ Map.elems occurs where
    step :: Char -> Map Char Int -> Map Char Int
    step = Map.alter (maybe (Just 1) (Just . succ))
    counts = BS8.foldr step Map.empty s
    occurs :: Map Int Int
    occurs = foldr (Map.alter (maybe (Just 1) (Just . succ)) . snd) Map.empty (Map.toList counts)
...
---- format
yesno :: Bool -> Input ()
yesno b = liftIO . putStrLn $ if b then "Yes" else "No"
