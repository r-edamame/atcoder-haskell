...
import           Data.Char (intToDigit, digitToInt, isSpace, isDigit, toLower)
...
solv :: ByteString -> ByteString -> Bool
solv s t
    | BS8.length t == 0 = True
    | BS8.length t == 1 && t == "X" = True
    | BS8.length s == 0 = False
    | BS8.head s == toLower (BS8.head t) = solv (BS8.tail s) (BS8.tail t)
    | otherwise = solv (BS8.tail s) t
