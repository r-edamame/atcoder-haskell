...
main = runInput $ do
    (h, w) <- readT
    mat <- readCharMat w h
    liftIO $ putStrLn . unwords . toList . V.map show $ solv h w mat

solv h w mat = V.accum (+) (V.replicate (min h w) 0) [(calcSize (x, y) - 1, 1) | x <- [0..w-1], y <- [0..h-1], isCenter (x, y)]
    where
    calcSize (x, y) = go 2 where
        go i
            | and [mat`at`((x+dx)`mod`w, (y+dy)`mod`h) == '#' | dx <- [-i, i], dy <- [-i, i]] = go (i+1)
            | otherwise = i - 1
    isCenter (x, y) = and [mat`at`(x+dx, y+dy) == '#' | dx <- [-1, 0, 1], dy <- [-1, 0, 1], abs dx == abs dy]
    at v (x, y)
        | x < 0 || w <= x || y < 0 || h <= y = '.'
        | otherwise = v!y!x
