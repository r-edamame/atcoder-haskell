...
main = runInput $ do
    (h, w) <- readT
    mata <- readCharMat w h
    matb <- readCharMat w h
    liftIO . putStrLn . yesno $ solv w h mata matb

solv w h mata matb = evalCont $ do
    forM_ [ (dx, dy) | dx <- [0..w-1], dy <- [0..h-1]] $ \(dx, dy) ->
        when (and [ mata!((y+dy)`mod`h)!((x+dx)`mod`w) == matb!y!x | x <- [0..w-1], y <- [0..h-1]]) (quit True)
    return False
