...
import           Control.Monad.Writer.Strict (WriterT(runWriterT), execWriterT, Writer, runWriter, execWriter, Sum (getSum))
...
main = runInput $ do
    n <- readInt
    let primes = eratosthenes 300000
    let l = V.length primes
    liftIO . print . sum $
        [ cm - bi
        | let am = fromMaybe (-1) $ lowerBound 0 (l-1) $ \i -> ((primes!i)^5) <= n
        , ai <- [0..am]
        , let a = primes ! ai
        , a^5 <= n

        , let bm = fromMaybe 0 $ lowerBound (ai+1) (l-1) $ \i -> (a^2) * (primes!i) <= n
        , bi <- [ai+1..min bm (l-2)]
        , let b = primes ! bi
        , b^3 <= n`div`(a^2) + 1

        , let cm = fromMaybe bi $ lowerBound (bi+1) (l-1) $ \i -> (a^2) * b * ((primes!i)^2) <= n
        ]
...
eratosthenes :: Int -> V.Vector Int
eratosthenes n = runST $ do
    tab <- V.thaw $ V.replicate (n+1) True
    void $ tab !*= (0, False)
    void $ tab !*= (1, False)
    forM_ @[] [2..isqrt n] $ \i ->
        whenM ((==Just True) <$> tab!*i) $ forM_ @[] [i*2, i*3 .. n] $ \j ->
            tab !*= (j, False)
    V.fromList . V.ifoldr (\i v xs -> if v then i : xs else xs) [] <$> V.freeze tab

isqrt :: Int -> Int
isqrt = ceiling . sqrt @Double . fromIntegral
