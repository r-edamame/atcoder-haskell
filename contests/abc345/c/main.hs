...
solv :: VC -> Int
solv s = n + sum (map (fold) $ L.inits $ toList counts) where
    counts = foldr (Map.alter (Just . maybe 1 (+1))) Map.empty s
    total = s.len
    ap = ((total - 1)^(2::Int) + total) `div` 2
    n = fromEnum (maximum (toList counts) > 1)
