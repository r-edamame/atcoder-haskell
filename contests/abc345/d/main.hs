...
data Perm a = Leaf a | Perm a [Perm a]

perm :: [a] -> [Perm a]
perm [] = []
perm [a] = [Leaf a]
-- perm [(x,y)] = if' (x/=y) ((Perm (y, x) []):) [Perm (x,y) []]
perm xs = map ((\(r,rs) -> Perm r (perm rs)) . ext xs) [0..length xs - 1]
-- perm xs = fmap (ext xs) [0..xs.len-1] >>= \((x,y),rs) -> if' (x/=y) ((Perm (y,x) (perm rs)):) [Perm (x,y) (perm rs)]

ext :: [a] -> Int -> (a, [a])
ext [] _ = undefined
ext (x:xs) 0 = (x, xs)
ext (x:xs) n = second (x:) (ext xs (n-1))

pmap :: (a -> [a]) -> Perm a -> [Perm a]
pmap f (Leaf a) = Leaf <$> f a
-- pmap f (Perm a []) = flip Perm [] <$> f a
pmap f (Perm a ps) = flip Perm (join $ map (pmap f) ps) <$> f a

ptoList :: Perm a -> [[a]]
ptoList (Leaf a) = [[a]]
ptoList (Perm a ps) = ps >>= \p -> (a:) <$> ptoList p
...
solv :: Int -> Int -> Int -> V.Vector (Int, Int) -> Bool
solv n h w ts = any (`check`sini) choices where
    sini = Set.fromList [(x, y) | x <- [0..w-1], y <- [0..h-1]]
    -- choices = L.permutations (toList ts) >>= traverse (\(x,y) -> if' (x/=y) ((y,x):) [(x,y)])
    put (tw, th) s
        | (x,y) <- Set.elemAt 0 s,
            s' <- s `Set.difference` Set.fromList [(x',y') | x' <- [x..x+tw-1], y' <- [y..y+th-1]],
            Set.size s - Set.size s' == (tw*th) = Just s'
        | otherwise = Nothing

    choices = perm (toList ts) >>= pmap (\(x,y) -> if' (x/=y) ((y,x):) [(x, y)])
    check _ s
        | Set.null s = True
    check (Leaf t) s
        | Just s' <- put t s = Set.null s'
        | otherwise = False
    check (Perm t ps) s
        | Just s' <- put t s = any (`check`s') ps
        -- | (x, y) <- Set.elemAt 0 s,
        --   s' <- s `Set.difference` Set.fromList [(x',y') | x' <- [x..x+tw-1], y' <- [y..y+th-1]],
        --   Set.size s - Set.size s' == (tw*th) = if_ (Set.null s') True (any (`check`s') ps)
        | otherwise = False
    -- check [] s = Set.null s
    -- check ((tw, th):ps) s
    --     | Set.null s = True
    --     | (x, y) <- Set.elemAt 0 s,
    --         s' <- s `Set.difference` Set.fromList [(x', y') | x' <- [x..x+tw-1], y' <- [y..y+th-1]],
    --         Set.size s - Set.size s' == tw*th = check ps s'
    --     | otherwise = False

if_ :: Bool -> a -> a -> a
if_ True = const
if_ False = flip const
if' :: Bool -> (a -> a) -> (a -> a)
if' b f = if_ b f id
