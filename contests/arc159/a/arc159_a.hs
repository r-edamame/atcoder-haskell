...
import           Control.Lens ((.~), (%~), _1, _2, _3, both, Bifunctor (bimap))
...
import           Control.Monad.Trans.Cont (ContT(ContT, runContT), evalContT, Cont, cont, runCont)
...
main = do
    result <- flip fmap BS8.getContents $ runInput $ do
        ...
        q <- readInt
        qs <- readTs1 q
        return $ solv n mat qs
    forM_ result print

solv :: Int -> Mat Int -> V.Vector (Int, Int) -> V.Vector Int
solv n (mat, ix) = V.map (f . bimap (`mod`n) (`mod`n)) where
    f (s, t) =
        let initial = Seq.fromList $ zip (filter (\i -> mat ! ix i s == 1) [0..n-1]) [1,1..] in
        debug "result" $ runBfs (go t) (-1) fst initial
    go :: Int -> (Int, Int) -> BFS Int (Int, Int) Identity ()
    go t (p, cnt) = do
        if p == t then quit cnt else do
            forM_ @[] [0..n-1] $ \to ->
                when (mat ! ix to p == 1) (queue (debug "to" to, cnt+1))
