...
main = runInput $ do
    ...
    q <- readInt
    qs <- V.replicateM q ((,) <$> readChar <*> readChar)
    liftIO . putStrLn . toList $ solv n s q qs

solv :: Int -> VC -> Int -> V.Vector (Char, Char) -> VC
solv n s q qs = V.map (table.at . c2i) s
    where
        table = runST $ do
            t <- V.thaw ['a'..'z']
            forM_ qs $ \(c,d) ->
