...
main = runInput $ do
    ...
    q <- readInt
    qs <- readTs1 q
    printV (BS8.pack . show . succ) $ solv n ps q qs

solv :: Int -> VI -> Int -> V.Vector (Int, Int) -> VI
solv n ps q qs = V.map (\(a, b) -> if poss.at a < poss.at b then a else b) qs where
    poss = V.map snd . V.modify (VH.sortBy compare) $ V.zip ps [0..n-1]
