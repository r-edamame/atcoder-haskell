...
solv :: Int -> VI -> Int
solv n as = zs + (sum . map (\i -> (i*i - i)`div`2) $ Map.elems cnts) where
    sqrs = [i*i | i <- [1..448]]
    zc = V.length $ V.filter (==0) as
    zs = ((2*n - zc - 1) * zc) `div` 2
    withoutSqrs = V.map (\a -> a`div`(maximum [s | s <- sqrs, s <= a, a`mod`s==0])) (V.filter (/=0) as)
    cnts = V.foldr (Map.alter (Just . maybe 1 succ)) Map.empty withoutSqrs
