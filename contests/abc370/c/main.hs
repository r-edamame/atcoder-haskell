...
main = runBuffered $ do
    ...
    t <- toVC <$> readWord
    let
        lt = V.map fst $! V.filter snd $! V.zip [0..s.len] (V.zipWith (<) s t)
        gt = V.map fst $! V.filter snd $! V.zip [0..s.len] (V.zipWith (>) s t)
        ans :: [ByteString] = runST $ execWriterT $ do
            tb <- lift $ V.thaw s
            for_ (gt <> V.reverse lt) $ \i -> do
                lift $ tb.set i (t.at i)
                lift (V.freeze tb) >>= tell . pure . fromVC
    printLines ans
    return ()
...
instance (Output a, Foldable f) => Output (FOutput f a) where
    toBs (FO sc sep vs) =
        ...
        in if sc then toBs (length vs) <> (if length vs > 0 then "\n" else "") <> bd else bd
