...
main = runInput $ do
    (n, k) <- readT
    ps <- readInts1 n
    liftIO $ print $ solv n k ps

vwindow :: Int -> VI -> [VI]
vwindow n v
    | v.len < n = []
    | otherwise = V.slice 0 n v : vwindow n (V.tail v)

solv :: Int -> Int -> VI -> Int
solv n k ps = flip evalState (Set.fromList $ V.toList $ head choices) $ foldM go maxBound choices where
    ixes = fmap fst . V.modify (VH.sortBy cmp) . V.zip [0..n] $ ps
    cmp (_, x1) (_, y1) = compare x1 y1
    choices = vwindow k ixes
    go :: Int -> VI -> State (Set Int) Int
    go ans c = do
        s <- State.get
        let s' = Set.insert (c.at (k-1)) s
        let result = Set.findMax s' - Set.findMin s'
        State.put $ Set.delete (c.at 0) s'
        return $ min ans result
...
---- format
yesno :: Bool -> Input ()
yesno True = liftIO $ putStrLn "Yes"
yesno False = liftIO $ putStrLn "No"
