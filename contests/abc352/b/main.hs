...
main = runInput $ do
    s <- readWord
    t <- readWord
    liftIO . putStrLn . unwords . map show $ solv s t

solv :: ByteString -> ByteString -> [Int]
solv s t = reverse $ go (debug "s" (BS8.unpack s)) (debug "t" (BS8.unpack t)) 1 [] where
    go [] _ i ans = ans
    go (x:xs) (y:ys) i ans
        | x == y = go xs ys (succ i) (i:ans)
        | otherwise = go (x:xs) ys (succ i) ans
    go _ _ _ _ = error "unreached"
