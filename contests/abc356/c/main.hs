...
main = runInput $ do
    ...
    m <- readInt
    k <- readInt
    ts <- V.replicateM m $ do
        c <- readInt
        (,) <$> V.replicateM c readInt1 <*> readChar
    liftIO $ print $ solv n m k ts

solv :: Int -> Int -> Int -> V.Vector (VI, Char) -> Int
solv n m k ts = (V.filter pass candidates).len where
    candidates = V.replicateM n ([True, False] :: V.Vector Bool)
    sat c (t, r) = (if r == 'o' then (>=k) else (<k)) . V.length . V.filter id $ V.map (c V.!) t
    pass c = and $ V.map (sat c) ts
...
search :: (PushPop pp, Hashable a, Monad m, Foldable f) => (Elem pp -> m (f (Elem pp))) -> (Elem pp -> a) -> pp -> m ()
search f uq = go HSet.empty where
    go dup pp = case pop pp of
        Just (a, pp')
            | HSet.member (uq a) dup -> go dup pp'
            | otherwise -> f a >>= go (HSet.insert (uq a) dup) . foldr push pp'
        Nothing -> return ()
searchFind :: (Monad m, PushPop pp, Hashable (Elem pp)) => (Elem pp -> m (Either b [Elem pp])) -> pp -> m (Maybe b)
searchFind f ini = flip runContT (return . const Nothing) $ search f' id ini where
