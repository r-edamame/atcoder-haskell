...
main = runInput $ do
    (n, m) <- readT
    liftIO $ print $ solv n m

solv :: Int -> Int -> Mint
solv n m = go 1 m 0 where
    go d m' ans
        | debug "m'" m' == 0 || n < d = ans
        | m' .&. 1 == 0 = go (d*2) (m'`shiftR`1) ans
        | otherwise = let
            dn = (n+1) `div` (2 * debug "d" d) * d + max 0 ((n+1)`mod`(d*2) - d)
            in go (d*2) (m'`shiftR`1) (ans + (debug "add" $ mint dn))
...
mint :: Integral i => i -> Mint
mint = Mint . (`mod` mintMod) .  fromIntegral
