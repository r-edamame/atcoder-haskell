...
main = runInput $ do
    (n, m) <- readT
    as <- readInts m
    xss <- V.replicateM n $ V.replicateM m readInt
    yesno $ solv n m as xss

solv :: Int -> Int -> VI -> V.Vector VI -> Bool
solv n m as xss = and (V.zipWith (>=) total as) where
    total = foldl1 (V.zipWith (+)) xss
...
---- format
yesno :: Bool -> Input ()
yesno = liftIO . BS8.putStrLn . (!!) ["No", "Yes"] . fromEnum
