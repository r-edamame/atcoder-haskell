...
main = do
    result <- flip fmap BS8.getContents $ runInput $ do
        ...
        w <- readInt
        mat <- readIntMat w h
        return . showIntMat h w $ transpose w h mat
    BS8.putStr result
...
yesno True = "Yes"
yesno False = "No"
showCharMat :: Int -> Int -> Mat Char -> ByteString
showCharMat w h = BS8.unlines . map (fst . BS8.unfoldrN w unconsV) . takes w . fst
showIntMat :: Int -> Int -> Mat Int -> ByteString
showIntMat  w h = BS8.unlines . map (BS8.unwords . V.foldr (\i r -> showBS i : r) []) . takes w . fst
...
---- transpose
transpose :: Int -> Int -> Mat a -> Mat a
transpose w h (src, ix) = runST $ do
    let ix' x y = h*y + x
    mat <- MV.new (w*h)
    forM_ @[] [0 .. h - 1] $ \x ->
        forM_ @[] [0 .. w - 1] $ \y ->
            MV.write mat (ix' x y) (src ! ix y x)
    (,ix') <$> V.freeze mat
