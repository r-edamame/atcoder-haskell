...
main = do
    result <- flip fmap BS8.getContents $ runInput $ do
        n <- readInt
        s <- V.replicateM n readChar
        return . unwords . map show $ solv n s
    putStrLn result

solv :: Int -> V.Vector Char -> [Int]
solv n s = toList $ V.foldl (\s (i, c) -> if c == 'L' then s :|> (i-1) else (i-1) :<| s) (Seq.singleton n) (V.reverse $ V.zip [1..n] s)
