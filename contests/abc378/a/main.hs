...
main = runBuffered $ do
    as <- V.replicateM 4 readInt
    let
        step st a
            | HSet.member a st = tell (Sum @Int 1) >> pure (HSet.delete a st)
            | otherwise = pure (HSet.insert a st)
        Sum ans = execWriter $ foldM step HSet.empty as
    print ans
    return ()
