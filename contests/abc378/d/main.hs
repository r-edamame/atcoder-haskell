...
main = runBuffered $ do
    ...
    w <- readInt
    k <- readInt
    mat <- V.replicateM h (V.replicateM w readChar)
    let
        f :: (T, Set T) -> Writer (Sum Int) [(T, Set T)]
        f ((x, y), st)
            | Set.size st == (k+1) = seq () tell (Sum 1) >> pure []
            | otherwise = pure [((x', y'), Set.insert (x', y') st)
                | dx <- [-1,0,1]
                , dy <- [-1,0,1]
                , dx+dy==1 || dx+dy==(-1)
                , let x' = x+dx
                ...
                , 0 <= y' && y' < h
                , (mat.at y').at x' /= '#'
                , not (Set.member (x', y') st)
                ]
        blanks = [(x, y) | x <- [0..w-1], y <- [0..h-1], (mat.at y).at x == '.']
        Sum ans = fold [execWriter (searchM f id (Seq.singleton (b, Set.singleton b))) | b <- blanks]
    print ans
    return ()
