...
type P = (Int, Int)
solv :: Int -> Mat Char -> Int
solv n mat = fromMaybe (-1) $ searchFind f (uq . snd) ([(0, ps)] :: Seq ((Int, (P, P)))) where
    ps = (\[p1, p2] -> (p1, p2)) [(x, y) | x <- [0..n-1], y <- [0..n-1], (mat.at y).at x == 'P']
    ok x y
        | x < 0 || n <= x || y < 0 || n <= y = False
        | otherwise = (mat.at y).at x /= '#'
    move (x, y) (dx, dy) = let p@(x', y') = (x+dx, y+dy) in if ok x' y' then p else (x, y)
    dirs = [(1,0), (-1,0), (0,1), (0,-1)]
    uq (p1, p2) = (min p1 p2, max p1 p2)
    f :: (Int, (P, P)) -> Either Int [(Int, (P, P))]
    f (c, (p1, p2))
        | p1 == p2 = Left c
        | otherwise = Right $ map (\d -> (c+1, (move p1 d, move p2 d))) dirs
    -- g = debug "g" $ V.fromList $
    --     [g' (x0, y0) (x1, y1)
    --         | x0 <- [0..n-1]
    --         , y0 <- [0..n-1]
    --         , x1 <- [0..n-1]
    ...
    --         ]
    -- g' p0 p1
    --     | uncurry ok p0 && uncurry ok p1 = [(p0', p1')
    --         | d <- dirs
    --         , let p0' = move p0 d
    --         , let p1' = move p1 d
    --         , p0 /= p0' || p1 /= p1'
