...
main = runInput $ do
    ...
    w <- readInt
    n <- readInt
    liftIO . BS8.putStrLn . showCharMat $ solv h w n

type T = (Int, Int)

solv :: Int -> Int -> Int -> Mat Char
solv h w n = runST $ do
    map <- V.replicateM h (V.thaw (V.replicate w '.'))
    step (0,0) (0, -1) map n
    traverse V.freeze map
    where
        move (x, y) (dx, dy) = ((x+dx)`mod`w, (y+dy)`mod`h)
        step :: T -> T -> V.Vector (MV.MVector s Char) -> Int -> ST s ()
        step _ _ _ 0 = return ()
        step (x, y) (dx, dy) map n = do
            t <- (map.at y).at x
            let t' = if t == '.' then '#' else '.'
            let d' = if t == '.' then (-dy, dx) else (dy, -dx)
            (map.at y).set x t'
            step (move (x,y) d') d' map (n-1)
