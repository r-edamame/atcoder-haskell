...
import           Control.Monad.Trans.Writer.CPS (WriterT, runWriterT, execWriterT, runWriter, execWriter)
...
data Q = Add Int Int | ShowCards Int | ShowBoxes Int
...
main = runInput $ do
    ...
    qs <- V.replicateM q $ do
        c <- readInt
        case c of 
            1 -> Add <$> readInt <*> readInt
            2 -> ShowCards <$> readInt
            3 -> ShowBoxes <$> readInt
            _ -> undefined
    liftIO $ forM_ (solv n qs) $ BS8.putStrLn . BS8.unwords . map showBS

solv :: Int -> V.Vector Q -> [[Int]]
solv n qs = execWriter $ V.ifoldM_ step (HMap.empty, HMap.empty) qs where
    step (cards, boxes) ix q = case q of
        Add i j -> return (upsert j (Set.insert (i,ix)) (Set.singleton (i,ix)) cards, upsert i (Set.insert j) (Set.singleton j) boxes)
        ShowCards i -> do
            Writer.tell . return @[] . map fst . Set.elems $ cards HMap.! i
            return (cards, boxes)
        ShowBoxes i -> do
            Writer.tell . return @[] . Set.elems $ boxes HMap.! i
            return (cards, boxes)

upsert k f d = HMap.alter (Just . maybe d f) k
