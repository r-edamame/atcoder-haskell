...
import           Control.Monad (replicateM, forM_, when, foldM, filterM, foldM_)
...
newtype TC r a b = TC { unTC :: (a -> b -> r) -> r }
a >< b = TC (\c -> c a b)

data Q = Add Int | Remove | Print
...
main = runInput $ do
    ...
    qs <- V.replicateM q $ do
        ...
        case i of
            ...
            3 -> return Print
            _ -> undefined
    liftIO $ forM_ (solv qs) print

solv qs = execWriter $ foldM_ step (Mint 1 >< Seq.singleton 1) qs where
    step :: (forall r. TC r Mint (Seq Int)) -> Q -> Writer [Int] (forall r. TC r Mint (Seq Int))
    step (!total, !digit, queue@(p :<| ps)) q = case q of
        Remove -> return (total - (digit * Mint p), digit`div`10, ps)
        Add a -> return (total*10+Mint a, digit*10, queue |> a)
        Print -> do
            Writer.tell (return @[] total)
            return (total, digit, queue)
    step _ _ = undefined
...
instance Integral Mint where
    quotRem m n = let q = m * revMint n in
        (q, m-q*n)
    toInteger (Mint n) = toInteger n
revMint :: Mint -> Mint
revMint (Mint n) = let
    (a, b) = extGcd mintMod n
    in
        Mint ((b + mintMod) `mod` mintMod)
