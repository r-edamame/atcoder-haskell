...
main = runInput $ do
    ...
    matA <- readIntMat n n
    matB <- readIntMat n n
    liftIO . putStrLn . yesno  $ solv n matA matB

solv :: Int -> Mat Int -> Mat Int -> Bool
solv n matA matB = same dotsA || same (rotate dotsA) || same (rotate (rotate dotsA)) || same (rotate (rotate (rotate dotsA))) where
    rotate dots = [(n-1-y, x) | (x, y) <- toList dots ]
    same a = all (==1) [matB!y!x | (x, y) <- a]
    dotsA = [(x, y) | x<-[0..n-1], y<-[0..n-1], matA!y!x == 1]
