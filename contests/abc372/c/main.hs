...
instance HasField "slice" (MV.MVector s a) (Int -> Int -> ST s (V.Vector a)) where
    getField mv l r = V.freeze $ MV.slice l (r-l) mv

main = runBuffered $ do
    n <- readInt
    q <- readInt
    s <- toVC <$> readWord
    !xcs <- V.replicateM q ((,) <$> readInt1 <*> readChar)
    let
        sc v = V.filter (\i -> v.slice i (i+3) == ['A','B','C']) [0..v.len-3]
        contains = not . V.null . sc
        !ans :: VI = runST $ flip evalStateT (V.length $ sc s) $ do
            tb <- lift $ V.thaw s
            for xcs $ \(!x, !c) -> do
                !before <- contains <$> V.freeze (MV.slice (max 0 (x-2)) (min n (x+3)) tb)
                lift $ tb.set x c
                !after <- contains <$> V.freeze (MV.slice (max 0 (x-2)) (min n (x+3)) tb)
                when (before /= after) $ State.modify' (if after then succ else pred)
                State.get
    print . FO False "\n" $ ans
    return ()
...
debug label a
    | debugging = seq (trace (concat @[] [label, ": ", show a])) a
    | otherwise = a
