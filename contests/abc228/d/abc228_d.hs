...
import           Control.Monad.State.Strict (StateT (StateT, runStateT), State, runState, evalStateT)
...
main = runInput $ do
    q <- readInt
    ts <- readTs q
    liftIO . putStr . unlines . map (show @Int) $ solv q ts

solv :: Int -> V.Vector (Int, Int) -> [Int]
solv q ts = execWriter $ runStateT (foldM step HMap.empty ts) (Set.singleton (0, n-1)) where
    n = 2^20
    -- step :: HashMap Int Int -> (Int, Int) -> WriterT [Int] (State (Set (Int, Int))) (HashMap Int Int)
    step mp q = do
        case q of
            (1, x') -> do
                let x = x'`mod`n
                segs <- State.gets (debug "segs")
                let m1 = case Set.lookupLE (x, n+1) segs of
                        Just seg@(s, t)
                            | t < x -> Nothing
                            | otherwise -> Just $ do
                                State.modify (Set.delete $ debug "deleting 1" seg)
                                when (s < x) $ State.modify (Set.insert (s, x-1))
                                when (x < t) $ State.modify (Set.insert (x+1, t))
                                return $ debug "map" (HMap.insert (max s x) x' mp)
                        Nothing -> Nothing
                let m2 = case Set.lookupGE (x, -1) segs of
                        Nothing -> Nothing
                        Just seg@(s, t) -> Just $ do
                            State.modify (Set.delete $ debug "deleting 2" seg)
                            when (x < t) $ State.modify (Set.insert (x+1, t))
                            return (HMap.insert s x' mp)
                let m3 = case Set.lookupGE (-1, 0) segs of
                        Nothing -> Nothing
                        Just seg@(s, t) -> Just $ do
                            State.modify (Set.delete $ debug "deleting 3" seg)
                            when (s /= t) $ State.modify (Set.insert (s+1, t))
                            return (HMap.insert s x' mp)
                fromJust $ (m1 <|> m2 <|> m3)
            (2, x) -> do
                Writer.tell $ [fromMaybe (-1) (HMap.lookup (x`mod`n) mp)]
                return $ debug "map" mp
            _ -> undefined
