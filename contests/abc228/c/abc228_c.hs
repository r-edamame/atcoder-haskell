...
main = runInput $ do
    n <- readInt
    k <- readInt
    pss <- V.replicateM n $ V.replicateM 3 readInt
    liftIO . putStr . unlines . toList . V.map yesno  $ solv n k pss

solv n k pss = V.map ((<=300).(`subtract`kth)) sums where
    sums = V.map sum pss
    sorted = V.modify (VH.sortBy (flip compare)) $ sums
    kth = sorted!(k-1)
