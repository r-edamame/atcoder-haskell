...
main = runInput $ do
    ...
    m <- readInt
    bs <- toList <$> readInts m
    l <- readInt
    cs <- toList <$> readInts l
    q <- readInt
    xs <- toList <$> readInts q
    mapM_ yesno $ solv as bs cs xs

solv :: [Int] -> [Int] -> [Int] -> [Int] -> [Bool]
solv as bs cs xs = map (flip Set.member (Set.fromList [a+b+c | a <- as, b <- bs, c <- cs])) xs
