...
main = runInput $ do
    ...
    n <- readInt
    sss <- V.replicateM n $ do
        a <- readInt
        V.replicateM a $ V.fromList . BS8.unpack <$> readWord
    liftIO $ print $ solv t n sss

solv :: VC -> Int -> V.Vector (V.Vector VC) -> Int
solv t n sss = fm . getMinPlus $ dp
        [(n, t.len)]
        ((0,0), (n, t.len))
        (\case
            (0, 0) -> Just (MinPlus 0)
        ...
        sp
    where
        fm i = if i == maxBound then -1 else i
        a `tailOf` b
            | b.len < a.len = False
            | otherwise = V.slice (b.len-a.len) a.len b == a
        sp (i, j) = sss.at (i-1)
            |> V.filter (`tailOf`t.slice 0 j)
            |> V.map (\s -> (MinPlus 1, (i-1, j-s.len)))
            |> toList
            |> (:) (MinPlus 0, (i-1, j))
...
newtype MinPlus = MinPlus { getMinPlus :: Int } deriving (Eq, Ord)
instance Semiring MinPlus where
    srzero = MinPlus maxBound
    srone = MinPlus 0
    srplus (MinPlus x) (MinPlus y) = MinPlus (min x y)
    srmul (MinPlus x) (MinPlus y)
        | x == maxBound || y == maxBound = srzero
        | otherwise = MinPlus (x + y)
instance Show MinPlus where
    show = show . getMinPlus
...
dp target rng isTrivial subproblems = runST $ do
    ...
    used <- MA.newArray rng False
    foldl srplus srzero <$> traverse (go table used) target
    where
        go :: forall s. STA.STArray s i r -> STA.STArray s i Bool -> i -> ST s r
        go table used i
            | Just a <- isTrivial i = return a
            | otherwise = do
                u <- MA.readArray used i
                if u then
                    MA.readArray table i
                else do
                    a <- foldl srplus srzero <$> traverse (\(s, i') -> srmul s <$> go table used i') (subproblems i)
                    MA.writeArray table i a
                    MA.writeArray used i True
                    return a
