...
data Cmd = Insert Int Int | Delete Int
...
main = runInput $ do
    ...
    qs <- V.replicateM q $ do
        i <- readInt
        case i of
            1 -> Insert <$> readInt <*> readInt
            2 -> Delete <$> readInt
            _ -> undefined
    printV (BS8.pack . show) $ solv as qs

type L = HashMap Int (Int, Int)

solv :: V.Vector Int -> V.Vector Cmd -> VI
solv as qs = V.fromList . toL $ foldl exec ini qs where
    exec list cmd
        | Insert i x <- cmd = ins i x list
        | Delete i <- cmd = del i list
    ini :: L
    ini = HMap.fromList $
        [(minBound, (0, as.at 0)), (maxBound, (V.last as, 0))] ++ [(as.at i, (p, n)) | i <- [0..as.len-1], let p = if_ (i==0) minBound (as.at (i-1)), let n = if_ (i==as.len-1) maxBound (as.at (i+1))]
    ins i a list = let (prv, nxt) = fromJust (HMap.lookup i list) in
        list
            |> HMap.adjust (second (const a)) i
            |> HMap.adjust (first (const a)) nxt
            |> HMap.insert a (i, nxt)
    del i list = let (prv, nxt) = fromJust (HMap.lookup i list) in
        list
            |> HMap.adjust (second (const nxt)) prv
            |> HMap.adjust (first (const prv)) nxt
            |> HMap.delete i
    toL list = go minBound where
        go i = let (_, nxt) = fromJust (HMap.lookup i list) in
            if_ (nxt==maxBound) [] $ nxt : go nxt
...
printV tobs v = liftIO $ do
    -- print v.len
    BS8.putStrLn . BS8.unwords . toList $ V.map tobs v
