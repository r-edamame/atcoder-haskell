...
import           Control.Monad (replicateM, forM_, when, foldM, filterM, mapM)
...
main = runBuffered $ do
    s <- toVC <$> readWord
    let
        ps = runExcept $ mapM (\v -> when (v.len /= 2) (throwE ()) >> pure (v.at 0)) (V.group s)
    yesno $ if
        | Left _ <- ps -> False
        | Right cs <- ps -> length cs == Set.size (Set.fromList cs)
