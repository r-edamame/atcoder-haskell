...
import           Control.Monad (replicateM, forM, forM_, when, foldM, filterM)
...
main = runBuffered $ do
    ...
    s <- toVC <$> readWord
    let
        step :: T -> Char -> Writer Max T
        step t c
            | c == '1' = case t of
                    One a -> return $ One (succ a)
                    Two _ a -> do
                        tell (Max (a * 2 + 1))
                        return (One 1)
            | c == '2' = case t of
                ...
                Two l a
                    | a + 1 == l -> do
                        tell (Max (l * 2 + 1))
                        return (One 0)
                    | otherwise -> return $ Two l (succ a)
            | otherwise = case t of
                One a
                    | a == 0 -> return $ One 0
                    | otherwise -> return $ Two a 0
                Two _ a -> do
                    tell $ Max (a * 2 + 1)
                    return $ One 0
        Max ans = execWriter $ foldM step (One 0) s
    print ans
    return ()
