...
newtype Max = Max Int
instance Semigroup Max where Max a <> Max b = Max (max a b)
instance Monoid Max where mempty = Max 0

main = runBuffered $ do
    n <- readInt
    as <- V.replicateM n readInt
    let
        ps :: VI -> VI
        ps v = V.generate ((n+1)`div`2) (\i -> if (2*i+1) < v.len && v.at (2*i) == v.at (2*i+1) then v.at (2*i) else -1)
        go :: VI -> Int -> Int -> HashSet Int -> Writer Max ()
        go v l r st = case v V.!? r of
            Nothing -> pure ()
            Just ra -> if
                | ra /= (-1) && not (HSet.member ra st) -> tell (Max (r-l+1)) >> go v l (r+1) (HSet.insert ra st)
                | l < r -> go v (l+1) r (HSet.delete (v V.! l) st)
                | otherwise -> go v (r+1) (r+1) HSet.empty
        calc v = execWriter (go v 0 0 HSet.empty)
        Max ans = calc (ps as) <> calc (ps $ V.tail as)
    print (ans*2)
    return ()
