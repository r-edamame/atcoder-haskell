...
main = runInput $ do
    ...
    k <- readInt
    rs <- readInts n
    liftIO $ forM_ (solv n k rs) $ \v ->
        putStrLn . concat . L.intersperse " " . map show . toList $ v

solv :: Int -> Int -> VI -> [VI]
solv n k rs = go (Just (V.replicate n 1)) where
    next :: VI -> Maybe VI
    next v = fmap f (L.find @[] (\j -> v.at j < rs.at j) [n-1, n-2 .. 0]) where
        f i = V.accum (flip const) v ((i, v.at i + 1) : [(l, 1)| l <- [i+1..n-1]])
    go = \case
        Nothing -> []
        Just v
            | sum v `mod` k == 0 -> v : go (next v)
            | otherwise -> go (next v)
