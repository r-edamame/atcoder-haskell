...
main = runInput $ do
    ...
    m <- readInt
    as <- readInts n
    liftIO $ print $ solv n m as

solv :: Int -> Int -> VI -> Int
solv n m as = fromJust (HMap.lookup 0 ini) + go ini 1 where
    modCumu = debug "modCumu" $ (as <> V.init as).cumulative (\a b -> (a + b) `mod` m) 0
    -- ini = debug "ini" $ V.accumulate (+) (V.replicate m 0) (V.map (,1) (modCumu.slice 1 n))
    ini = foldr (HMap.adjust succ) (HMap.fromList [(i, 0)|i<-[0..m-1]]) (modCumu.slice 1 n)
    go mp i
        | i == n = 0
        | otherwise = let
            -- mp' = debug "v'" $ V.unsafeAccum (+) v [(modCumu.at i, -1), (modCumu.at (n-1+i), 1)]
            mp' = HMap.adjust succ (modCumu.at (n-1+i)) (HMap.adjust pred (modCumu.at i) mp)
            in fromJust (HMap.lookup (modCumu.at i) mp') + go mp' (i+1)
