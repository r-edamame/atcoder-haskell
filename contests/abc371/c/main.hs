...
import           Data.Foldable (foldrM, foldlM, toList, traverse_, maximumBy, minimumBy, for_)
...
import           Data.Traversable (traverse, mapAccumR, mapAccumL, for)
...
main = runBuffered $ do
    !n <- readInt
    !mg <- readInt
    !ges <- force <$!!> (V.replicateM mg $ (,) <$!!> readInt1 <*> readInt1)
    !mh <- readInt
    !hes <- force <$!!> (V.replicateM mh $ (,) <$!!> readInt1 <*> readInt1)
    !a <- force <$!!> (for @V [1..n-1] $ \i -> V.replicateM (n-i) readInt)
    let
        (!g, !h) = both (V.accumulate (flip (:)) (V.replicate n []) . (>>= (\(u, v) -> [(u, v), (v, u)]))) $!! (ges, hes)
        calcCost :: VI -> Int
        calcCost arng = getSum $!! execWriter $ do
            for_ @[] [0..n-1] $ \i -> do
                let
                    !hto = Set.fromList $ h.at i
                    !gto = Set.fromList $!! map (fromJust . flip V.findIndex arng . (==)) $ g.at (arng.at i)
                    !diff = Set.union hto gto Set.\\ Set.intersection hto gto
                for_ diff $ \j -> when (i < j) $! tell (Sum ((a.at i).at (j - 1 - i)))
        !ans = minimum $!! map (calcCost . V.fromList) $!! L.permutations [0..n-1]
    print ans
...
instance (Output a, Foldable f) => Output (FOutput f a) where
    toBs (FO sc sep vs) =
        ...
        in if sc then toBs (length vs) <> bd else bd
...
instance HasField "len" (MV.MVector s a) Int where
    getField = MV.length
instance Show a => HasField "at" (V.Vector a) (Int -> a) where
    getField v = debug "at" . (v V.!)
