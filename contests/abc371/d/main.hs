...
main = runBuffered $ do
    n <- readInt
    xs <- V.replicateM n readInt
    ps <- V.replicateM n readInt
    q <- readInt
    let
        pcumu = V.scanl' (+) 0 ps
    for_ @[] [1..q] $ \_ -> do
        (l, r) <- readT
        let
            li = maybe 0 succ $ lowerBound 0 (n-1) (\i -> xs.at i < l)
            ri = maybe 0 succ $ lowerBound 0 (n-1) (\i -> xs.at i <= r)
        print $ pcumu.at ri - pcumu.at li
