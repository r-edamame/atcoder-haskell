...
main = runInput $ do
    n <- readInt
    ds <- readTs n
    printV (BS8.pack . show) $ solv n ds

solv :: Int -> V.Vector (Int, Int) -> VI
solv n ds = go cbya maxBound [] where
    cbya = foldr (\(i, d) -> Map.alter (Just . maybe [(i, snd d)] ((i, snd d):)) (fst d)) Map.empty (V.zip [1..n] ds)
    go :: Map Int [(Int, Int)] -> Int -> [Int] -> VI
    go mp upc result
        | Map.null mp = V.modify (VH.sortBy compare) . V.fromList $ result
        | otherwise = let
            ((a, cs), mp') = Map.deleteFindMax mp
            remains = filter ((<=upc) . snd) cs
            in go mp' (minimum $ upc : map snd remains) (map fst remains ++ result)
