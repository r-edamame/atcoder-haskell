...
main = runInput $ do
    n <- readInt
    ds <- V.replicateM n $ (,) <$> readWord <*> readInt
    liftIO $ BS8.putStrLn $ solv n ds

solv :: Int -> V.Vector (ByteString, Int) -> ByteString
solv n ds = sortedNames.at (total`mod`n) where
    sortedNames = V.modify (VH.sortBy compare) $ V.map fst ds
    total = V.sum . V.map snd $ ds
