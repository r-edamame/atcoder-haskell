...
solv :: Int -> Mint
solv n = fst $ go n n' p where
    n' = mint (fromIntegral n)
    dig = length (show n)
    p = mint $ 10 ^ dig
    go :: Int -> Mint -> Mint -> (Mint, Mint)
    go i acc pw
        | i == 1 = (acc, pw)
        | otherwise =
            let (acc', pw') = go (i`div`2) acc pw
                twosum = acc' + (acc' * pw')
            in if even i then (twosum, pw'*pw') else (twosum * pw + acc, pw'*pw'*pw)
