...
main = runInput $ do
    n <- readInt
    liftIO $ forM_ @[] (solv n) BS8.putStrLn

solv :: Int -> [ByteString]
solv n = [ln y | y <- [0..3^n-1]] where
    ln :: Int -> ByteString
    ln y = BS8.pack $ toList $ flip V.map [0..3^n-1] $ \x -> c x y n
    c x y k
        | k == 0 = '#'
        | x`div`(s k) == 1 && y`div`(s k) == 1 = '.'
        | otherwise = c (x`mod`(s k)) (y`mod`(s k)) (k-1)
        where
            s k' = 3 ^ (k'-1)
