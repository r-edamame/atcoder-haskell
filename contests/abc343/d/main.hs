...
main = runInput $ do
    (n, t) <- readT
    qs <- V.replicateM t $ (,) <$> readInt1 <*> readInt
    printV (BS8.pack . show ) $ solv n t qs

solv :: Int -> Int -> V.Vector (Int, Int) -> VI
solv n t qs = V.fromList (go (Map.fromList [(i, 0)|i<-[0..n-1]]) (Map.fromList [(0, n)]) (toList qs)) where
    go ps cnts [] = []
    go ps cnts ((a, b):qs') = let
        p = fromJust (Map.lookup a ps)
        ps' = Map.adjust (+b) a ps
        cnts' = cnts
            |> Map.alter (\(Just c) -> if c==1 then Nothing else Just (pred c)) p
            |> Map.alter (Just . maybe 1 succ) (p+b)
        in Map.size cnts' : go ps' cnts' qs'
