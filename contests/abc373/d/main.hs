...
main = runBuffered $! do
    !n <- readInt
    !m <- readInt
    !es <- V.replicateM m ((,,) <$> readInt1 <*> readInt1 <*> readInt)
    let
        !to = V.accumulate (flip (:)) (V.replicate n []) (V.map (\(!u,!v,!w) -> (u, (v, w))) $! (es >>= \(u,v,w) -> [(u,v,w), (v,u,-w)]))
        step :: MV s Int -> Int -> ST s [Int]
        step tb !p = do
            !vp <- tb.at p
            for (to.at p) $ \(t, w) -> do
                whenM ((== minBound) <$> tb.at t) $! do
                    tb.set t (vp+w)
                pure t
        !ans = ($! V.replicate n minBound) $! V.modify $ \(!tb) -> do
            for_ @[] [0..n-1] $ \(!i) -> do
                whenM ((==minBound) <$> tb.at i) $! do
                    tb.set i 0
                    void $ searchM (step tb) id $! ([i] :: [Int])
    print ans
    return ()
