...
main = runBuffered $ do
    s <- V.map (\c -> ord c - ord 'A') . toVC <$> readWord
    let
        pos = runST $ do
            tb <- V.thaw (V.replicate 26 0)
            for_ (V.zip [0..25] s) $ \(i, c) -> tb.set c i
            V.freeze tb
        step :: Int -> Int -> Writer (Sum Int) Int
        step cur nx = tell (Sum $ abs (pos.at cur - pos.at nx)) >> pure nx
        Sum ans = execWriter $ foldM @[] step 0 [1..25]
    print ans
    return ()
