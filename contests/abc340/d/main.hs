...
main = runInput $ do
    n <- readInt
    ts <- V.replicateM (n-1) $ (,,) <$> readInt <*> readInt <*> readInt1
    liftIO $ print $ solv n ts

solv :: Int -> V.Vector (Int, Int, Int) -> Int
solv n ts = fromJust $ searchFind f snd ([(0, 0)] :: Set (Int, Int)) where
    f :: (Int, Int) -> Either Int [(Int, Int)]
    f (c, i)
        | i == n-1 = Left c
        | otherwise = let (a, b, x) = ts.at i in Right [(c+a, i+1), (c+b, x)]
