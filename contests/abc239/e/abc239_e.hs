...
main = do
    ...
    let result = flip runInput input $ do
        ...
        g <- udg n <$> readTs1 (n-1)
        qs <- readTs1 q
        return . unlines . map show . V.toList $ solv n g xs qs
    putStr result

solv :: Int -> G Int -> V.Vector Int -> V.Vector (Int, Int) -> V.Vector Int
solv n g xs = fmap (\(v, k) -> table ! v ! k) where
    table :: V.Vector (V.Vector Int)
    table = dfs n g xs

dfs :: Int -> G Int -> V.Vector Int -> V.Vector (V.Vector Int)
dfs n g vs = runST $ do
    mat <- V.thaw $ V.replicate n V.empty
    go mat 0 (-1)
    V.freeze mat
    where
        go :: MV.MVector s (V.Vector Int) -> Int -> Int -> ST s ()
        go mat t p
            | p /= (-1) && V.length (g!t) == 1 = do
                MV.write mat t $ V.singleton (vs ! t)
            | otherwise = do
                forM_ (g!t) $ \s ->
                    if s == p then return ()
                    else go mat s t
                v <- V.take 20 . V.modify (VH.sortBy (flip compare)) . V.cons (vs!t) . foldl1 mappend . debug <$> forM (g!t) (\i -> if i==p then return mempty else MV.read mat i)
                MV.write mat t v
