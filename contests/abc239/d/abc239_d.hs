...
main = do
    ...
    let result = flip runInput input $ do
        ...
        c <- readInt
        d <- readInt
        let f ok = if ok then "Takahashi" else "Aoki"
        return . f . isJust $ L.find @[] (\x -> all @[] (not . isPrime . (+x)) [c..d]) [a..b]
        return $ show h
    putStrLn result

h :: V.Vector (Maybe Int)
h = runST $ do
    heap@(MHeap (mv, _, _)) <- newHeap 10 HeapAsc
    insertHeap 1 heap
    insertHeap 3 heap
    a <- popHeap heap
    insertHeap 2 heap
