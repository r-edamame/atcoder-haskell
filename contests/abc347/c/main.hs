...
main = runInput $ do
    ...
    (a, b) <- readT
    ds <- readInts1 n
    liftIO . putStrLn . yesno $ solv n a b ds

solv :: Int -> Int -> Int -> VI -> Bool
solv n a b ds = (mods.len <= 1) || (mxd >= b) where
    ab = a + b
    mods = V.fromList . toList $ V.foldr (Set.insert . (`mod`ab)) Set.empty ds
    mxd = V.maximum $ [ab - 1 - mods.at (mods.len - 1) + mods.at 0] V.++ V.map pred (V.zipWith (-) (V.tail mods) mods)
