...
main = runInput $ do
    ...
    b <- readInt
    c <- readInt
    liftIO $ BS8.putStrLn $ maybe "-1" showT $ solv a b c

solv :: Int -> Int -> Int -> Maybe (Int, Int)
solv a b c = runST $ do
    dp <- V.replicateM 61 $ V.replicateM 61 $ V.thaw $ V.replicate 61 (Nothing :: Maybe (Int, Int))
    ((dp.at 0).at 0).set 0 (Just (0, 0))
    -- 桁
    forM_ @[] [0..59] $ \i ->
        -- xのpopcount
        forM_ @[] [0..59] $ \j ->
            -- yのpopcount
            forM_ @[] [0..59] $ \k -> do
                cur <- ((dp.at i).at j).at k
                let bit = 1 `shiftL` i
                case cur of
                    Nothing -> return ()
                    Just (x', y')
                        | c .&. bit /= 0 -> do
                            ((dp.at (i+1)).at (j+1)).set k (Just (x'+bit, y'))
                            ((dp.at (i+1)).at j).set (k+1) (Just (x', y'+bit))
                        | otherwise -> do
                            ((dp.at (i+1)).at (j+1)).set (k+1) (Just (x'+bit, y'+bit))
                            ((dp.at (i+1)).at j).set k (Just (x', y'))
    ((dp.at 60).at a).at b
