...
main = do
    ...
    let result = flip runInput input $ do
        ...
        x <- readInt
        ts <- readTs n
        return $ yesno $ solv n x ts
    putStrLn result

solv :: Int -> Int -> V.Vector (Int, Int) -> Bool
solv n x ts = dp ! n ! x where
    dp :: V.Vector (V.Vector Bool)
    dp = runST $ do
        tb <- V.replicateM (n+1) $ V.thaw $ V.replicate 10200 False
        MV.write (tb ! 0) 0 True
        forM_ @[] [1..n] $ \i -> let (s, t) = ts ! (i-1) in
            forM_ @[] [0..x-1] $ \j ->
                whenM (MV.read (tb ! (i-1)) j) $ do
                    MV.write (tb ! i) (j+s) True
                    MV.write (tb ! i) (j+t) True
        traverse V.freeze tb
