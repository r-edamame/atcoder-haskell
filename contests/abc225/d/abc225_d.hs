...
data Q = Connect Int Int | Disconnect Int Int | Print Int deriving Show
...
main = runInput $ do
    ...
    qs <- V.replicateM q $ do
        i <- readInt
        case i of
            1 -> Connect <$> readInt1 <*> readInt1
            2 -> Disconnect <$> readInt1 <*> readInt1
            3 -> Print <$> readInt1
            _ -> undefined
    liftIO $ forM_ (solv n q qs) $ \v -> putStrLn . unwords . map show . (V.length v:) $ toList v

solv n q qs = runST $ execWriterT $ do
    nexts <- V.thaw $ V.replicate n (-1)
    prevs <- V.thaw $ V.replicate n (-1)
    forM_ qs (step nexts prevs)
    where
        getList nexts r = do
            Just nxt <- nexts !* r
            if nxt == -1 then return [succ r] else (succ r:) <$> getList nexts nxt
        getRoot prevs i = do
            Just r <- prevs !* i
            if r == -1 then return i else getRoot prevs r
        step nexts prevs query = case query of
            Connect x y -> do
                nexts !*= (x, y)
                void $ prevs !*= (y, x)
            Disconnect x y -> do
            ...
            Print x -> do
                r <- getRoot prevs x
                l <- V.fromList <$> getList nexts r
                Writer.tell $ return @[] l
...
runHeapDP f initial r = runContT (go (Set.fromList initial)) (const (return r)) wher
    go set = do
