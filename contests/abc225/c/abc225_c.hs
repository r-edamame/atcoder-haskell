...
main = runInput $ do
    (n, m) <- readT
    mat <- readIntMat1 m n
    liftIO . putStrLn . yesno $ solv m n mat

solv w h mat = diff7 && asc where
    diff7 = and [(mat!succ y!x) - (mat!y!x) == 7 | y <- [0..h-2], x <- [0..w-1]]
    asc = and [d`mod`7+1 == d1`mod`7 | x <- [0..w-2], let d = mat!0!x, let d1=mat!0!succ x]
