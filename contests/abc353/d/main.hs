...
main = runInput $ do
    n <- readInt
    as <- readInts n
    liftIO $ print $ solv n as

solv :: Int -> VI -> Mint
solv n as = snd $ V.foldl (flip step) (digitsCounts, 0) $ V.zip [0..n-1] as where
    digitsCounts :: Map Int Mint
    digitsCounts = foldr (Map.alter (Just . maybe 1 succ) . d) Map.empty as
    d = length . show
    step :: (Int, Int) -> (Map Int Mint, Mint) -> (Map Int Mint, Mint)
    step (i, a) (cs, total) = (cs', total + first + secondary) where
        cs' = Map.alter (fmap pred . (>>= nothingIf (==1))) (d a) cs
        secondary = mint $ a * i
        first = Map.foldlWithKey f 0 cs'
        f acc dig cnt = acc + mint a * (10 ^ dig) * cnt
