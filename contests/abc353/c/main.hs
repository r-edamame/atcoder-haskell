...
solv :: Int -> VI -> Int
solv n as = V.sum $ V.map f [1..n-1] where
    sorted = V.modify (VH.sortBy compare) as
    sums = sorted.cumulative (+) 0
    f i =
        let
            a = sorted.at (i-1)
            t = fromMaybe n $ upperBound i (n-1) (\k -> sorted.at k + a >= md)
        in a * (n-i) + sums.at n - sums.at i - ((n-t) * md)
