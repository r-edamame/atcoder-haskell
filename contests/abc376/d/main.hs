...
main = runBuffered $ do
    n <- readInt
    m <- readInt
    abs <- V.replicateM m readT1
    let
        to = V.accumulate (flip V.cons) (V.replicate n V.empty) abs
        f :: (Int, Int) -> Either Int [(Int, Int)]
        f (p, c) = V.toList <$> for (to.at p) (\b -> if b == 0 then Left (c+1) else Right (b, c+1))
        ans = fromMaybe (-1) $ searchFind f fst (Seq.singleton (0, 0))
    print ans
    return ()
