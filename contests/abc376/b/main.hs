...
main = runBuffered $ do
    n <- readInt
    q <- readInt
    hts <- V.replicateM q $ (,) <$> readChar <*> readInt1
    let
        ini = (0, 1)
        in' a s1 s2 = min s1 s2 < a && a < max s1 s2
        diff a b = max a b - min a b
        len a other to = diff a $ head [to' | to' <- (to +) <$> [-n, 0, n], all @[] (\d -> not (in' (other+d) a to')) [-n, 0, n]]
        step :: T -> (Char, Int) -> Writer (Sum Int) T
        step (l, r) (h, t) = case h of
            'L' -> tell (Sum (len l r t)) >> pure (t, r)
            _ -> tell (Sum (len r l t)) >> pure (l, t)
        Sum ans = execWriter $ foldM step ini hts
    print ans
    return ()
