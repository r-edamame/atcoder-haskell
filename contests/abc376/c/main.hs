...
type StepM s e m = s -> e -> m s

main = runBuffered $ do
    n <- readInt
    as <- toList . V.modify (VH.sortBy (flip compare)) <$> V.replicateM n readInt
    bs <- toList . V.modify (VH.sortBy (flip compare)) <$> V.replicateM (n-1) readInt
    let
        step :: StepM ([Int], Maybe Int) Int Maybe
        step (a:as', m) b = if
            | a <= b -> pure (as', m)
            | Nothing <- m -> step (as', Just a) b
            | otherwise -> Nothing
        step _ _ = Nothing
        ans = case foldM step (as, Nothing) bs of
            Just ([], Just a) -> a
            Just ([a], Nothing) -> a
            _ -> -1
    print ans
