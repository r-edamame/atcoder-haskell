...
main = runInput $ do
    (n, q) <- readT
    as <- readInts n
    qs <- V.replicateM q $ (,,) <$> readInt <*> readInt <*> readInt
    liftIO . putStr . unlines . map show $ solv n q as qs

solv :: Int -> Int -> VI -> V.Vector (Int, Int, Int) -> [Int]
solv n q as qs = map getSum $ go ini (toList qs) where
    ini = segMk . toList . V.map Sum $ as
    go _ [] = []
    go seg (q:qs)
        | (0, p, x) <- q = go (segMod seg p (<>Sum x)) qs
        | (1, l, r) <- q = segSum seg l r : go seg qs
        | otherwise = go seg qs
