...
import           Control.Lens ((.~), (%~), _1, _2, _3, _1')
...
import           Data.Monoid (mempty, Sum (Sum, getSum))
...
main :: IO ()
main = runInput $ do
    let seg = segMk $ map Sum [0..7 :: Int]
    liftIO . print . getSum $ segSum seg 0 7
    liftIO . print . getSum $ segSum seg 1 8
    liftIO . print . getSum $ segSum seg 3 5
    liftIO . print . getSum $ segSum seg 4 8
    liftIO . print . getSum $ segSum seg 0 4
    -- (n, t) <- readT
    -- s <- V.fromList . BS8.unpack <$> readWord
    -- xs <- readInts n
    -- liftIO $ print $ solv n t s xs

solv :: Int -> Int -> VC -> VI -> Int
solv n t s xs = V.sum (V.map (\x -> lb (x+2*t) - lb (x-1)) ps) where
    sxs = V.zip s xs
    ps = V.map snd . V.filter ((=='1') . fst) $ sxs
    ns = V.modify (VH.sortBy compare) . V.map snd . V.filter ((/='1') . fst) $ sxs
    lb p = fromMaybe (-1) $ lowerBound 0 (ns.len-1) ((<=p) . (ns V.!))
...
segPut (SegLeaf _) _ a = SegLeaf a
segPut (SegNode ln _ cl cr) i a
    | i .&. (ln`shiftL`1) == 0 = let cl' = segPut cl i a in SegNode ln (cl'.value <> cr.value) cl' cr
    | otherwise = let cr' = segPut cr i a in SegNode ln (cl.value <> cr'.value) cl cr'
