...
main = runInput $ do
    ...
    m <- readInt
    k <- readInt
    liftIO $ print $ solv n m k

solv :: Int -> Int -> Int -> Int
solv n m k  = fromJust (upperBound 1 (maxBound - 1) (\i -> k <= i`div`n + i`div`m - 2*(i`div`l))) where
    l = product (factors n `whole` factors m)

factors :: Int -> [Int]
factors n = go n primes where
    go n' (p:ps)
        | n' < p*p = [n']
        | n'`mod`p == 0 = p : go (n'`div`p) (p:ps)
        | otherwise = go n' ps
    go _ _ = undefined
...
whole [] ys = ys
whole xs [] = xs
whole xs@(x:xs') ys@(y:ys')
    | x < y = x : whole xs' ys
    | x > y = y : whole xs ys'
    | otherwise = x : whole xs' ys'

primes :: [Int]
primes = fp (2 : [3,5..]) where
    fp (p:xs) = p : fp [x | x <- xs, x`mod`p /= 0]
    fp _ = undefined
