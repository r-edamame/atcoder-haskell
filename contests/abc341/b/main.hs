...
main = runInput $ do
    ...
    as <- readInts n
    sts <- readTs (n-1)
    liftIO $ print $ solv n as sts

solv :: Int -> VI -> V.Vector (Int, Int) -> Int
solv n as sts = V.foldl step (as.at 0) [0..n-2] where
    step c i = let (s, t) = sts.at i in t*(c`div`s) + as.at (i+1)
