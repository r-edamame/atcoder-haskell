...
main = runInput $ do
    ...
    mat <- readCharMat w h
    liftIO $ print $ solv h w n t mat


solv :: Int -> Int -> Int -> VC -> Mat Char -> Int
solv h w n t mat = V.length . V.filter (`check`0) $ ((,) <$> [0..w-1] <*> [0..h-1]) where
    ini = [((x,y),0)|x<-[0..w-1], y<-[0..h-1]]
    check (x, y) l
        | (mat.at y).at x == '#' = False
        | l == t.len = True
        | otherwise = flip check (l+1) $ case t.at l of
            'L' -> (x-1, y)
            'R' -> (x+1, y)
            'U' -> (x, y-1)
            'D' -> (x, y+1)
