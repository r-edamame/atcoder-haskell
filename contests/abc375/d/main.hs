...
import           Data.Char (intToDigit, digitToInt, isSpace, isDigit, ord)
...
main = runBuffered $ do
    s <- readWord
    let
        ixed = V.zip [0..s.len-1] (V.generate s.len (subtract (ord 'A') . ord . BS8.index s))
        z = V.replicate s.len 0
        counts :: V VI = V.map (\c -> (V.accumulate (+) z (V.map ((,1) . fst) $! V.filter ((==c) . snd) ixed)).cumulative (+) 0) [0..25]
        step :: Int -> Int -> Writer (Sum Int) ()
        step i c = tell $! Sum $! (counts.at c).at (i-1) * ((counts.at c).at s.len - (counts.at c).at i)
        Sum ans = execWriter $ for_ @[] [1..s.len] (\i -> for_ @[] [0..25] (\c -> step i c))
    print ans
    return ()
