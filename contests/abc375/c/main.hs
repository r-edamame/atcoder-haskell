...
main = runBuffered $! do
    n <- readInt
    mat <- V.replicateM n readWord
    let
        rot (x, y) = (y, n-1-x)
        f :: (Int, Int) -> Char
        f (x, y) = let
            d = minimum @[] [x+1, y+1, n-x, n-y]
            (x0, y0) = (L.iterate' rot (x, y)) !! (mod d 4)
            in BS8.index (mat.at y0) x0
        -- ans = fromVC <$> V.generate n (\y -> V.generate n (\x -> f (x, y)))
    for_ @[] [0..n-1] $ \y -> print $! BS8.pack $! map (\x -> f (x, y)) [0..n-1]
