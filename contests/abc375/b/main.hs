...
main = runBuffered $ do
    n <- readInt
    ps <- V.replicateM n readT
    let
        len :: T -> T -> Double
        f a b = fromIntegral ((a-b) ^ 2)
        len (x0, y0) (x1, y1) = sqrt (f x0 x1 + f y0 y1)
        step :: T -> T -> Writer (Sum Double) T
        step from to = tell (Sum $ len from to) >> pure to
        Sum ans = execWriter $ foldM step (0,0) (V.snoc ps (0,0))
    print ans
    return ()
