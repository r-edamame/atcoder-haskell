...
main = runInput $ do
    n <- readInt
    hs <- readInts n
    liftIO . putStrLn . unwords . map show $ solv n hs

solv :: Int -> VI -> [Int]
solv n hs = go 0 [] (toList hs) where
    go _ _ [] = []
    go t [] (h:hs') = (t+h+1) : go (t+h) [(h, 1)] hs'
    go t ((sh, sw):stk) (h:hs')
        | h <= sh = (t+h+1) : go (t+h) (if_ (h==sh) [(h,sw+1)] [(h,1),(sh,sw)] ++ stk) hs'
        | ((sh',sw'):stk') <- stk, sh' < h  = go (t+sw*(sh'-sh)) ((sh',sw+sw'):stk') (h:hs')
        | otherwise = go (t+sw*(h-sh)) ((h,sw):stk) (h:hs')
