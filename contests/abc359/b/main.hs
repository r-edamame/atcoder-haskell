...
solv :: Int -> VI -> Int
solv n as = length . filter (\[i, j] -> j - i == 2) $ toList ixes where
    ixes = foldr (\(i, a) -> Map.alter (Just . maybe [i] (i:)) a) Map.empty (V.zip [0..2*n-1] as)
...
-- overrides Prelude functions
print :: Show a => a -> Input ()
print = liftIO . Prelude.print
putStr :: String -> Input ()
putStr = liftIO . Prelude.putStr
putStrLn :: String -> Input ()
putStrLn = liftIO . Prelude.putStrLn
putStrBs :: ByteString -> Input ()
putStrBs = liftIO . BS8.putStr
putStrLnBs :: ByteString -> Input ()
putStrLnBs = liftIO . BS8.putStrLn
...
---- format
yesno :: Bool -> Input ()
yesno = putStrLn . (!!) ["No", "Yes"] . fromEnum
...
printV tobs v = do
    print v.len
    putStrBs . BS8.unlines . toList $ V.map tobs v
