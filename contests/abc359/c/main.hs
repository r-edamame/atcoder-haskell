...
solv :: (Int, Int) -> (Int, Int) -> Int
solv s' t'
    | left s' == left t' = 0
    | s.x > t.x = solv t s
    | s.y > t.y = solv s (t.x, s.y + s.y - t.y)
    | s.x == t.x = abs (s.y - t'.y)
    | s.y == t.y = 1 + (t.x - s'.x) `div` 2
    | otherwise = m + solv (s.x + m, s.y + m) t
    where
        (s, t) = case compare s.x t.x of
                EQ -> (s', t')
                GT -> (left s', right t')
                LT -> (right s', left t')
        m = min (t.x - s.x) (t.y - s.y)
