...
main = runInput $ do
    (n, k) <- readT
    s <- bs2cv <$> readWord
    liftIO $ print $ solv n k s

solv :: Int -> Int -> VC -> Mint
solv n k s =
    fromJust $ dp'
        (Right $ map ((n,) . fst) . filter (not . isP . snd) . zip [(0::Int)..] $ [0..1`shiftL`k-1])
        ((0, 0), (n, 1`shiftL`k - 1))
        trivial
        subproblems
    ...
    -- 回文判定
    isP :: Int -> Bool
    isP x = and [(x .&. (1`shiftL`i) /= 0) == (x .&. (1`shiftL`(k-1-i)) /= 0) | i <- [0..k`div`2-1]]
    can i c = s.at (i-1) == '?' || s.at (i-1) == c
    lst x = "BA" !! fromEnum (x.&.1 == 0)

    trivial :: (Int, Int) -> Maybe (Maybe Mint)
    trivial (0, j)
        | j == 0 = Just (Just 1)
        | otherwise = Just (Just 0)
    trivial (i, j)
        | i < k = Nothing
        | isP j = Just (Just 0)
        | not (can i (lst j)) = Just (Just 0)
        | otherwise = Nothing
    subproblems (i, j)
        | can i (lst j) = [(i-1, j') | c <- [0, 1`shiftL`(k-1)], let j' = c + j`shiftR`1]
        | otherwise = []


bs2cv :: ByteString -> VC
bs2cv bs = V.unfoldrN (BS8.length bs) BS8.uncons bs
cv2bs :: VC -> ByteString
cv2bs = BS8.pack . toList

instance Num a => Semiring (Maybe a) where
    srZero = Nothing
    srOne = Just 1
    srPlus Nothing Nothing = Nothing
    srPlus a b = Just (fromMaybe 0 a + fromMaybe 0 b)
    srMul a b = (*) <$> a <*> b
...
instance Semiring Mint  where
    srZero = Mint 0
    srOne = Mint 1
    srPlus = (+)
    srMul = (*)

dp :: forall r i. (Semiring r, Ix i, Eq r) => Either i [i] -> (i, i) -> (i -> Maybe r) -> (i -> [(r, i)]) -> r
dp target rng isTrivial subproblems = runST $ do
    table <- MA.newArray rng srZero
    case target of
        Left t -> go table t
        Right ts -> foldl srPlus srZero <$> traverse (go table) ts
    -- go table target
    where
        ...
        go table i
            ...
            | otherwise = do
                ...
                if v /= srZero then
                    ...
                    return a
dp' :: (Semiring r, Ix i, Eq r) => Either i [i] -> (i, i) -> (i -> Maybe r) -> (i -> [i]) -> r
dp' target rng isTrivial subproblems = dp target rng isTrivial (map (srOne,) . subproblems)
