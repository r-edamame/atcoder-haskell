...
import           Data.Foldable (foldrM, foldlM, toList, maximumBy)
...
solv :: V.Vector ByteString -> ByteString
solv ss = fst $ maximumBy (\a b -> compare (snd a) (snd b)) $ Map.toList counts where
    counts = V.foldl (\c s -> upsert s (1 :: Int) (+1) c) Map.empty ss

upsert :: (Ord k) => k -> a -> (a -> a) -> Map k a -> Map k a
upsert k d f = Map.alter (maybe (Just d) (Just . f)) k
