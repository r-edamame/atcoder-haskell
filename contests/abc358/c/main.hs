...
import           Data.Bits (shiftL, shiftR, (.&.), (.|.), xor, Bits (popCount))
...
main = runInput $ do
    (n, m) <- readT
    ss <- V.replicateM n $ (V.fromList . BS8.unpack <$> readWord)
    liftIO $ print $ solv n m ss

solv :: Int -> Int -> V.Vector VC -> Int
solv n m ss = minimum . map popCount . filter sat $ [1..2^n-1 :: Int] where
    digs = V.iterateN m (*2) 1
    filled = 2 ^ m - 1
    ns = V.map (V.sum . V.zipWith (*) digs . V.map (fromEnum . (=='o'))) ss
    sat i = (==filled) . foldl1 (.|.) . map (ns V.!) . filter (\b -> i .&. (1`shiftL`b) /= 0) $ [0..n-1]

choices :: [a] -> [[a]]
choices [] = [[]]
choices (x:xs) = choices xs ++ map (x:) (choices xs)
...
choice n l
    ...
    where
        go [] = []
        go (x:xs) = map (x:) (choice (pred n) xs) ++ go xs
