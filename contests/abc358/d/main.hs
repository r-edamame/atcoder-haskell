...
main = runInput $ do
    ...
    as <- readInts n
    bs <- readInts m
    liftIO $ print $ solv n m as bs

solv :: Int -> Int -> VI -> VI -> Int
solv n m as bs = go (toList as') (toList bs') 0 where
    as' = V.modify (VH.sortBy compare) as
    bs' = V.modify (VH.sortBy compare) bs
    go (x:xs) (y:ys) ans
        | x < y = go xs (y:ys) ans
        | otherwise = go xs ys (ans + x)
    go _ [] ans = ans
    go [] _ _ = -1
