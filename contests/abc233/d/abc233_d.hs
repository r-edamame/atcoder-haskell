...
import           Control.Monad.Writer.Strict (WriterT(runWriterT), Writer, runWriter, execWriter, Sum (Sum, getSum))
...
main = do
    result <- flip fmap BS8.getContents $ runInput $ do
        ...
        k <- readInt
        as <- readInts n
        return . show $ solv n k as
    putStrLn result

solv :: Int -> Int -> V.Vector Int -> Int
solv n k as = getSum . execWriter $ foldM @[] step HMap.empty [1..n] where
    cum = cumulative as
    step :: HashMap Int Int -> Int -> Writer (Sum Int) (HashMap Int Int)
    step mp i = do
        let mp' = upsertH (cum!(i-1)) 1 (+1) mp
        Writer.tell $ Sum $ fromMaybe 0 $ HMap.lookup ((cum!i)-k) mp'
        return mp'


upsertH :: (Eq k, Hashable k) => k -> a -> (a -> a) -> HashMap k a -> HashMap k a
upsertH k d f = HMap.alter (maybe (Just d) (Just . f)) k
