...
main = do
    result <- flip fmap BS8.getContents $ runInput $ do
        n <- readInt
        x <- readInt
        ls <- V.replicateM n $ readInt >>= readInts 
        return . show $ solv n ls x
    putStrLn result

solv :: Int -> V.Vector (V.Vector Int) -> Int -> Int
solv n ls x = getSum . execWriter $ dfs 0 1 where
    dfs :: Int -> Int -> Writer (Sum Int) ()
    dfs depth p
        | p > x = return ()
        | depth == n =
            when (p == x) $ Writer.tell (Sum 1)
        | otherwise =
            forM_ (ls!depth) $ \b -> dfs (depth+1) (b*p)
