...
main = runInput' $ do
    ...
    m <- readInt
    as <- readInts n
    ps <- V.replicateM m ((,,) <$> readInt1 <*> readInt1 <*> readInt)
    return . putStrLn . unwords . map show $ solv n m as ps

solv :: Int -> Int -> VI -> V.Vector (Int, Int, Int) -> [Int]
solv n m as ps = tail . map snd . L.sort . execWriter $ searchM f snd ([(0, 0)] :: Set (Int, Int))  where
    qs' = ps >>= \(u, v, b) -> [(u, (v, b)), (v, (u, b))]
    g = V.accumulate (flip (:)) (V.replicate n []) qs'
    f :: (Int, Int) -> Writer [(Int, Int)] [(Int, Int)]
    f (c, i) = do
        tell [(i, as.at i + c)]
        return $ map (\(v, b) -> (c+b+as.at i, v)) (g.at i)
...
runInput' :: Input (IO ()) -> IO ()
runInput' i = BS8.getContents >>= (fromJust<$>) . runMaybeT . evalStateT i >>= liftIO
---- util
