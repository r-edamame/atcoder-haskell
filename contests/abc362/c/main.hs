...
main = runInput $ do
    ...
    case ans of
        ...
        Just v -> do
            yesno True
            liftIO . putStrLn . unwords . map show . toList $ v

solv :: Int -> V.Vector (Int, Int) -> Maybe VI
solv n ss = if 0 < lsum || rsum < 0 then Nothing else Just (snd $ mapAccumL f lsum ss) where
    lsum = sum . V.map fst $ ss
    rsum = sum . V.map snd $ ss
    f :: Int -> (Int, Int) -> (Int, Int)
    f sm (l, r)
        | sm == 0 = (0, l)
        | otherwise = let w = min (r-l) (abs sm) in (sm + w, l+w)
