...
main :: IO ()
main = runInput $ do
    s <- toCharVector <$> readWord
    t <- toCharVector <$> readWord
    liftIO . print $ solv s t
...
solv s t = runST $ do
    ...
    dp <- V.replicateM (sl+1) $ V.thaw (V.replicate (tl+1) 0)
    forM_ @[] [1..sl] $ \si ->
        forM_ @[] [1..tl] $ \ti -> do
            Just p <- ((+ b2i (s!pred si == t!pred ti))<$>) <$> (dp!pred si) !* pred ti
            Just m <- max <$> (dp!pred si)!*ti <*> (dp!si)!*pred ti
            (dp!si) !*= (ti, max p m)
    fromJust <$> (dp!sl) !* tl
...
debugIntMat label mat = trace (concat @[] [label, "\n", BS8.unpack $ showIntMat 0 0 mat]) mat
...
(!*) :: MV.MVector s a -> Int -> ST s (Maybe a)
m !* i = traverse (MV.read m) $ nothingIf (outOf m) i
(!*=) :: MV.MVector s a -> (Int, a) -> ST s (Maybe ())
m !*= (i, a) = traverse (flip (MV.write m) a) $ nothingIf (outOf m) i
(!*%) :: MV.MVector s a -> (Int, a -> a) -> ST s (Maybe ())
