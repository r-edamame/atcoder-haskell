...
main = runInput $ do
    ...
    n <- readInt
    ts <- readTs1 n
    liftIO . putStr . unlines . toList . V.map show $ solv d n ts

solv d n = imos d

imos :: Int -> V.Vector (Int, Int) -> V.Vector Int
imos n ts = V.init . snd $ mapAccumL (\s a -> (s+a, s+a)) 0 appears where
    appears = V.accumulate (+) (V.replicate (n+1) 0) $ V.concatMap (\(l, r) -> [(l, 1), (r+1, -1)]) ts
