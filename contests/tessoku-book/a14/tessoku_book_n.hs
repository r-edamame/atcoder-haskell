...
main = runInput $ do
    ...
    bs <- toList <$> readInts n
    cs <- toList <$> readInts n
    ds <- toList <$> readInts n
    let s = Set.fromList ds
    liftIO . putStrLn . yesno $ or [Set.member d s | a <- as, a<k, b <- bs, b<k-a, c <- cs, c<k-a-b, let d = k-a-b-c]
