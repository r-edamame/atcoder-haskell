...
main = runInput $ do
    (n, m) <- readT
    ass <- V.replicateM m $ readInts n
    let bs = V.map bitsToInt ass
    liftIO . print $ solv n bs
    -- liftIO . putStrLn $ solv

solv n bs = runST $ do
    dp <- V.thaw $ V.replicate (1`shiftL`n) (-1)
    dp !*= (0, 0)
    let f x y = if y == -1 then x else min x y
    forM_ bs $ \b ->
        forM_ (V.reverse [0..1`shiftL`n-2]) $ \i -> do
            Just s <- dp !* i
            when (s /= -1) (void $ dp !*% (i.|.b, f (s+1)))
    fromJust <$> dp !* (1`shiftL`n-1)



bitsToInt :: V.Vector Int -> Int
bitsToInt = go 0 where
    go n v = case unconsV v of
        Nothing -> n
        Just (x, v') -> go (n`shiftL`1 + x) v'
