...
main = runInput $ do
    ...
    as <- readInts (n-1)
    bs <- readInts (n-2)
    let ans = solv n as bs
    liftIO . print $ length ans
    liftIO . putStrLn . unwords . map show $ solv n as bs

solv n as bs = runST $ do
    dp <- V.thaw $ V.replicate (n+1) (0,0)
    dp !*= (2, (as!0, 1))
    !_ <- V.freeze dp
    forM_ @[] [3..n] $ \i -> do
        Just (c1, _) <- dp !* (i-1)
        ...
        let d1 = c1+(as!(i-2))
        let d2 = c2+(bs!(i-3))
        let j = if d1 < d2 then (d1, i-1) else (d2, i-2)
        dp !*= (i, j)
    tab <- V.map snd <$> V.freeze dp
    let go i r
            | i == 1 = 1 : r
            | otherwise = go (tab!i) (i:r)
    return $ go n []
