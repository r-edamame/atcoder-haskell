...
main = runInput $ do
    ...
    as <- readInts n
    let c = compress as
    let f a = (+1) . fromJust $ lowerBound 0 (V.length c - 1) (\i -> (c!i) <= a)
    liftIO . putStrLn . unwords . toList . V.map (show . f) $ as

solv = undefined

compress :: V.Vector Int -> V.Vector Int
compress v = V.fromList $ L.unfoldr (fmap Set.deleteFindMin . nothingIf Set.null) (Set.fromList . toList $ V.modify VH.sort v)
