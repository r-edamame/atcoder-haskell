...
seesaa :: Int -> String -> String
seesaa n [] = []
seesaa n (c:cs) = c' : seesaa n cs where
    Just i = BS8.findIndex (==c) hiragana
    c' = BS8.head $ BS8.drop ((i+n) `mod` BS8.length hiragana) hiragana

main :: IO ()
main = runInput $ do
    s <- 
    liftIO . unlines . map (flip seesaa )
