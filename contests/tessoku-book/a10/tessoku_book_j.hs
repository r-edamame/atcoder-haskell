...
main = runInput $ do
    ...
    d <- readInt
    qs <- readTs1 d
    liftIO . BS8.putStr . BS8.unlines . toList . V.map showBS $ solv n as qs

solv n as qs = V.map f qs where
    l = V.length as
    forward = cumulative max 0 as
    backward = cumulative max 0 $ V.reverse as
    f (l, r) = max (forward!l) (backward!((n-1)-r))
