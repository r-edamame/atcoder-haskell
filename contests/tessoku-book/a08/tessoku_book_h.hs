...
main = runInput $ do
    ...
    liftIO $ forM_ (solv mat qs) print

solv mat qs = V.map f qs where
    cum = cumuMat mat
    f (x1, y1, x2, y2) = cum!x2!y2 + cum!pred x1!pred y1 - cum!pred x1!y2 - cum!x2!pred y1

cumuMat :: V.Vector (V.Vector Int) -> V.Vector (V.Vector Int)
-- cumuMat mat = uncurry (flip V.snoc) $ mapAccumL f (V.replicate (w+1) 0) cumus where
--     h = V.length mat
--     w = V.length (mat!0)
--     cumus = V.map cumulative mat
--     f sm cumu = (V.zipWith (+) sm cumu, sm)

cumuMat mat = runST $ do
    let w = V.length $ mat!0
    let h = V.length $ mat
    cumu <- V.replicateM (h+1) $ V.thaw (V.replicate (w+1) 0)
    forM_ @[] [1..h] $ \y ->
        forM_ @[] [1..w] $ \x -> do
            v <- MV.read (cumu!y) (x-1)
            MV.write (cumu!y) x (v + mat!pred y!pred x)
    let !_ = debug "done" ()
    forM_ @V.Vector [1..w] $ \x -> 
        forM_ @V.Vector [1..h] $ \y -> do
            v <- MV.read (cumu!pred y) x
            MV.modify (cumu!y) (+v) x
    let !_ = debug "done" ()
    traverse V.freeze cumu
