...
main = runInput $ do
    ...
    liftIO . BS8.putStrLn . showIntMat 0 0 $ imos2d w h ps

solv = undefined

imos2d :: Int -> Int -> V.Vector (Int, Int, Int, Int) -> Mat Int
imos2d w h ps = V.init $ snd $ mapAccumL (\s a -> let v = V.zipWith (+) s a in (v, v)) (V.replicate w 0) horizontal where
    horizontal = V.map (snd . mapAccumL (\s a -> (s+a, s+a)) 0) appears
    appears = debugMatInt "appears" $ accumulate2d h (+) (V.replicate (h+1) (V.replicate (w+1) 0)) (V.concatMap (\(y1, x1, y2, x2) -> [((y1, x1), 1), ((y1, x2+1), -1), ((y2+1, x1), -1), ((y2+1, x2+1), 1)]) ps)

-- accumulate2d :: Show b => Int -> (a -> b -> a) -> Mat a -> V.Vector ((Int, Int), b) -> Mat a
accumulate2d :: Show b => Int -> (Int -> b -> Int) -> Mat Int -> V.Vector ((Int, Int), b) -> Mat Int
accumulate2d n f src mod = V.imap (\i v -> V.accum f v (each!i)) src where
    each = groupN (n+1) (fst.fst) (BF.first snd) mod


groupN :: Show b => Int -> (a -> Int)-> (a -> b)  -> V.Vector a -> V.Vector [b]
groupN n k f = debug "groupN" . V.fromList . HMap.elems . foldl step (HMap.fromList [(i, [])|i<-[0..n-1]]) where
    step mp v = HMap.adjust (f v:) (k v) mp

debugMatInt label mat = trace (concat @[] [label, "\n", BS8.unpack $ showIntMat 0 0 mat]) mat
-- debugMatInt label = id
...
showIntMat :: Int -> Int -> Mat Int -> ByteString
showIntMat  w h = BS.init . BS8.unlines . map (BS8.unwords . V.foldr (\i r -> showBS i : r) []) . toList
