...
import           Control.Monad.State.Strict (StateT (StateT, runStateT), evalStateT, State, state, runState, evalState)
...
import           Data.Foldable (foldrM, foldlM, toList, traverse_, maximumBy)
...
main = runInput $ do
    (n, w) <- readT
    ts <- readTs n
    liftIO . print $ solv n w ts

solv :: Int -> Int -> V.Vector (Int, Int) -> Int
solv n w ts = runST $ do
    dp <- V.thaw $ V.replicate (w+1) (-1)
    (dp, 0) !*= 0
    forM_ ts $ \(wi, v) -> do
        forM_ (V.reverse [0..w-1]) $ \i -> do
            Just v' <- dp !* i
            when (v' >= 0) $ do
                void $ (dp, i+wi) !*% max (v'+v)
    maximum <$> V.freeze dp



maxBy :: (a -> a -> Ordering) -> a -> a -> a
maxBy f a b = maximumBy @[] f [a, b]
...
outOf :: MV.MVector s a -> Int -> Bool
outOf m i = i<0 || MV.length m<=i
