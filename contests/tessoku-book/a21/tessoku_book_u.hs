...
main = runInput $ do
    n <- readInt
    ts <- V.replicateM n $ ((,) <$> readInt1 <*> readInt)
    liftIO . print $ solv n ts

solv :: Int -> V.Vector (Int, Int) -> Int
solv n ts = runST $ flip execStateT 0 $ do
    dp <- V.replicateM n $ V.thaw (V.replicate n 0)
    forM_ [(l, r) | r<-reverse [0..n-1], l<-[0..r]] $ \(l, r) -> do
        -- remove left
        let (pl, al) = fromMaybe (-1, 0) $ ts!?pred l
        prevl <- fromMaybe 0 <$> (dp!r)!*pred l
        let vl = if l<=pl&&pl<=r then prevl+al else prevl
        -- remove right
        let (pr, ar) = fromMaybe (-1, 0) $ ts!?succ r
        -- Maybe (MVector s Int)
        prevr <- fromMaybe 0 <$> (fmap join .traverse (!*l) $ (dp!?succ r))
        let vr = if l<=pr&&pr<=r then prevr+ar else prevr
