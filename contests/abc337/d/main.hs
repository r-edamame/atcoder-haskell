...
main = runInput $ do
    (h, w) <- readT
    k <- readInt
    mp <- V.replicateM h readWord
    liftIO $ print $ solv h w k mp

solv :: Int -> Int -> Int -> V ByteString -> Int
solv h w k mp = maybe (-1) (k-) $ max (maximum $ V.map mx mp) (maximum $ V.map mx trd) where
    trd = V.fromList . BS8.transpose . toList $ mp
    xc :: Char -> ByteString -> VI
    xc c bs = (V.map (\i -> fromEnum (bs.at i == c)) [0..bs.len-1 :: Int]).cumulative (+) 0
    mx :: ByteString -> Maybe Int
    mx bs = maximumM [ os.at (i+k) - os.at i | i <- [0..bs.len-k], xs.at (i+k) - xs.at i == 0] where
        xs = xc 'x' bs
        os = xc 'o' bs


maximumM :: [Int] -> Maybe Int
maximumM v = if_ (null v) Nothing (Just $ maximum v)
