...
main = runInput $ do
    n <- readInt
    as <- readInts n
    liftIO . putStrLn . unwords . toList . V.map show $ solv n as

solv :: Int -> VI -> VI
solv n as = V.cons s (V.unfoldrN (n-1) step s) where
    nexts = HMap.fromList $ zip (toList as) [1..n]
    s = fromJust $ HMap.lookup (-1) nexts
    step i = dup <$> HMap.lookup i nexts
    dup x = (x, x)
