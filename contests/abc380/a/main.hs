...
main = runBuffered $ do
    n <- readInt
    let digits = L.unfoldr (fmap (flip mod 10 &&& flip div 10) . wn (>0)) n
    let cnts = foldr (Map.alter (Just . maybe (1 :: Int) succ)) Map.empty digits
    yesno $ all @[] (\i -> Map.lookup i cnts == Just i) [1,2,3]

wn :: (a -> Bool) -> a -> Maybe a
wn p a = if p a then Just a else Nothing
...
instance Atcoder (WriterT [ByteString] (StateT ByteString Maybe)) where
...
instance Atcoder (StateT ByteString (MaybeT IO)) where
...
runBuffered :: WriterT [ByteString] (StateT ByteString Maybe) a -> IO a
runBuffered m = do
    s <- BS8.getContents
...
runInteractive :: StateT ByteString (MaybeT IO) a -> IO a
runInteractive m = do
