...
import           Data.Char (intToDigit, digitToInt, isSpace, isDigit, isLower, toUpper, toLower)
...
main = runBuffered $ do
    s <- readWord
    q <- readInt
    ks <- V.replicateM q readInt1
    let
        least2pow n = head . filter (n <=) $ iterate (*2) 1
        flipped n = if n == 1 then False else
                let hw = least2pow n `div` 2 in
                    not (flipped (n - hw))
        flipCase c = if isLower c then toUpper c else toLower c
    print $ flip fmap ks $ \k -> let
                (d, m) = divMod k s.len
                c = BS8.index s m
                in if flipped (d+1) then flipCase c else c
