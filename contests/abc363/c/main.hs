...
solv :: Int -> Int -> String -> Int
solv n k s = length . filter sat $ pm where
    pm = toList . HSet.fromList . map BS8.pack $ L.permutations s
    isP s' = all @[] (\i -> s'.at i == s'.at (k-1-i)) [0..k`div`2]
    sat :: ByteString -> Bool
    sat s' = and . map (not . isP) $ [s'.substr i (i+k) | i <- [0..n-k]]
