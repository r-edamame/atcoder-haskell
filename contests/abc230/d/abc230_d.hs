...
main = runInput $ do
    ...
    d <- readInt
    ws <- readTs n
    liftIO . print $ solv n d ws

-- solv :: p1 -> p2 -> V.Vector (Int, Int) -> t
solv n d _ws = go 0 minBound ws where
    ws = V.modify (VH.sortBy (\(_,t1) (_,t2) -> compare t1 t2)) _ws
    go cnt p v
        | V.null v = cnt
        | fst (V.head v) < p + d = go cnt p (V.tail v)
        | otherwise = go (cnt+1) (snd $ V.head v) (V.tail v)
