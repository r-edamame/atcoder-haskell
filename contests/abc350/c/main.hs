...
main = runInput $ do
    n <- readInt
    as <- readInts1 n
    printV (showT . both succ) $ solv n as

solv :: Int -> VI -> V.Vector (Int, Int)
solv n as = runST $ do
    arr <- V.thaw as
    ixes <- V.thaw . V.map fst . V.modify (VH.sortBy (\a b -> compare (snd a) (snd b))) . V.zip [0..n-1] $ as
    results <- newSTRef @[(Int, Int)] []
    forM_ @[] [0..n-1] $ \i -> do
        ii <- ixes.at i
        when (ii /= i) $ do
            t <- arr.at i
            ti <- ixes.at t
            modifySTRef results ((min ii ti, max ii ti):)
            ixes.swap i t
            arr.swap ii ti
    V.reverse . V.fromList <$> readSTRef results
