...
import           Control.Monad (replicateM, forM_, when, foldM, filterM, void)
...
main = runInput $ do
    (n, m) <- readT
    ps <- readTs1 m
    liftIO $ print $ solv n m ps
    liftIO $ print @Int 1

-- solv ::
solv :: Int -> Int -> V.Vector (Int, Int) -> Int
solv n m ps = runST $ do
    let g = udg n ps
    edges <- V.thaw $ V.replicate n (0 :: Int)
    uf <- newUF n
    forM_ @[] [0..n-1] $ \i -> do
        forM_ (g.at i) $ \to -> do
            edgesi <- edges.at i
            ...
            rooti <- uf.root i
            rootto <- uf.root to
            if rooti == rootto then
                edges.modify rooti succ
            else do
                void $ uf.unite i to
                rt <- uf.root i
                let (mr, mn, dr) = if rt == rooti then (rooti, edgesto, rootto) else (rootto, edgesi, rooti)
                edges.modify mr (+mn)
                edges.set dr 0
    uf.debug
    edges' <- debug "edges" <$> V.freeze edges
    V.sum <$> for (V.zip [0..n-1] edges') (\(i, e) -> do
        if e == 0 then
            return 0
        else do
            siz <- debug "siz" <$> uf.size i
            let perf = (siz * (siz - 1)) `div` 2
            return $ perf - e)
...
instance HasField "debug" (UnionFind s) (ST s ()) where
    getField (UnionFind(par, rank, siz)) = do
        !_ <- debug "par" <$> V.freeze par
        !_ <- debug "rank" <$> V.freeze rank
        !_ <- debug "siz" <$> V.freeze siz
        return ()
---- undirected graph
