...
main = runInput $ do
    (n, q) <- readT
    ts <- readInts q
    liftIO $ print $ solv n q ts

solv :: Int -> Int -> V.Vector Int -> Int
solv n q ts = Set.size $ foldl step initial ts where
    initial = Set.fromList [1..n]
    step :: Set Int -> Int -> Set Int
    step s t = if Set.member t s then Set.delete t s else Set.insert t s
