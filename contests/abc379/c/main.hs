...
import           Control.Monad.Trans.Except (ExceptT(ExceptT), runExceptT, throwE, runExcept)
...
main = runBuffered $ do
    (n, m) <- readT
    xas <- fmap (V.modify (VH.sortBy compare)) $ V.zip <$> V.replicateM m readInt <*> V.replicateM m readInt
    let
        sm a = (a * (a + 1)) `div` 2
        step (cur, hld, stp) (x, a) = do
            when (cur + hld  + 1 < x)  $ throwE (-1)
            return (x, hld - (x - cur - 1) + (a - 1), stp + sm hld - sm (hld - (x - cur)))
        ans = either id (\(_, hld, stp) -> if hld == 0 then stp else -1)
            $ runExcept (foldM step (0,0,0) (xas `V.snoc` (n, 0)))
    print ans
    return ()
...
instance Output a => Output (V.Vector a) where
    toBs = BS8.unwords . toList . fmap toBs
data FOutput f a = FO
    { showCnt :: Bool
...
instance (Output a, Foldable f) => Output (FOutput f a) where
    toBs (FO sc sep vs) = BS8.concat
        [ if sc then toBs (length vs) <> "\n" else ""
        , BS8.concat $ L.intersperse sep (toBs <$> toList vs)
        ]
...
type Buffered = ContT () (WriterT [ByteString] (StateT ByteString Maybe))
instance Atcoder Buffered where
    readInt = lift . lift . StateT $ BS8.readInt . dropSpaces
    readChar = lift . lift . StateT $ BS8.uncons . dropSpaces
    readWord = lift . lift . StateT $ pure . BS8.break isSpaceChar8 . dropSpaces
    print a = lift $ Writer.tell [toBs a]
    quit = ContT (const $ pure ())
...
instance Atcoder Interactive where
    ...
    print a = liftIO $ BS8.putStrLn (toBs a) >> hFlush stdout

runBuffered :: Buffered a -> IO ()
runBuffered m = do
    s <- BS8.getContents
    let res = runStateT (runWriterT (runContT m (const $ pure ()))) s
    case res of
        Nothing -> error "error occurred"
        Just ((a, out), s) -> do
            BS8.putStr (BS8.unlines out)
            return a
