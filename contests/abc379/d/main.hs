...
import           Control.Monad (replicateM, forM_, when, foldM, filterM, replicateM_)
...
main = runInteractive $ do
    ...
    let
        calc h = do
            (sq, offset) <- debug "(sq, offset)" <$> State.get
            if offset < h then pure 0 else
                case sq of
                    Seq.Empty -> return 0
                    (t, n) :<| sq' -> do
                            State.put (sq', offset - t)
                            (n +) <$> calc h
        go :: (Monad m, Atcoder m) => StateT (Seq (Int, Int), (Int, Int)) m ()
        go = do
            c <- lift readInt
            case c of
                1 -> State.modify (debug "1" . second (second succ))
                2 -> do
                    t <- lift readInt
                    (ts, (offset, pending)) <- State.get
                    State.put . debug "2" $ (ts :|> (t, pending), (offset + t, 0))
                _ -> do
                    h <- lift readInt
                    (ts, (offset, pending)) <- State.get
                    let (a, (ts', offset')) = runState (calc h) (ts, offset)
                    State.put . debug "3" $ (ts', (offset', pending))
                    lift $ print a
    evalStateT (replicateM_ q go) ([], (0, 0))
    return ()
...
type Buffered = WriterT [ByteString] (StateT ByteString Maybe)
...
segDump sh (SegNode _ a cl cr) = sh a : zipWith (\b c -> b ++ " " ++ c) (segDump sh cl) (segDump sh cr)
