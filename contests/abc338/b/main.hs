...
main = runInput $ do
    s <- readWord
    liftIO $ putStrLn . L.singleton $ solv s

solv :: ByteString -> Char
solv s = fst $ maximumBy (\a b -> (snd a `compare` snd b) `compare` (fst a `compare` fst b)) $ Map.toList cnts where
    cnts = BS8.foldr (Map.alter (Just . maybe 1 succ)) Map.empty s

agg :: (Hashable k, Monoid m, Foldable f) => (a -> k) -> (a -> m) -> f a -> HashMap k m
agg k v = foldr (\e -> HMap.alter (Just . maybe (v e) (v e<>)) (k e)) HMap.empty

cat :: [(Int, Int)] -> HashMap Int [Int]
cat = agg fst (return . snd)
