...
main = runInput $ do
    ...
    as <- readInts n
    bs <- readInts n
    liftIO $ print $ solv n qs as bs

solv :: Int -> VI -> VI -> VI -> Int
solv n qs as bs = maximum (can qs bs : go qs 1)  where
    can qs' xs = minimum (V.zipWith div' qs' xs)
    div' _ 0 = maxBound
    div' x y = div x y
    tk qs' xs = sequenceA $ V.zipWith (\x y -> if x < y then Nothing else Just (x-y)) qs' xs
    go qs' i
        | Just qs'' <- tk qs' as = i + can qs'' bs : go qs'' (i+1)
        | otherwise = []
