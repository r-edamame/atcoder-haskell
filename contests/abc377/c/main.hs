...
main = runBuffered $ do
    n <- readInt
    m <- readInt
    ps <- V.replicateM m readT1
    let
        move :: [(Int, Int)]
        move = [(0, 0), (2, 1), (2, -1), (-2, 1), (-2, -1), (1, 2), (1, -2), (-1, 2), (-1, -2)]
        step st (x, y) = foldr HSet.insert st . filter (onMat n n) $ map ((+x) *** (+y)) move
        invalid = foldl step HSet.empty ps
    print $ (n * n) - HSet.size invalid
    return ()
