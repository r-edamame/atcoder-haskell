...
main = runBuffered $ do
    mat <- V.replicateM 8 (V.replicateM 8 readChar)
    let
        (row, col) = runST $ do
            r <- V.thaw (V.replicate 8 True)
            c <- V.thaw (V.replicate 8 True)
            forM_ @[] ((,) <$> [0..7] <*> [0..7]) $ \(x, y) -> do
                when ((mat.at y).at x == '#') $ do
                    r.set y False
                    c.set x False
            (,) <$> V.freeze r <*> V.freeze c
        ans = length $ filter (\(x, y) -> row.at y && col.at x) ((,) <$> [0..7] <*> [0..7])
    print ans
    return ()
