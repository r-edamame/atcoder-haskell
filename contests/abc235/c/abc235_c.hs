...
solv :: Int -> V.Vector Int -> V.Vector (Int, Int) -> V.Vector Int
solv n as = V.map f where
    mp = mapping n as
    f :: (Int, Int) -> Int
    f (x, k) = case HMap.lookup x mp of
        Nothing -> -1
        Just xs -> if V.length xs < k then -1 else xs ! (k-1)

mapping :: Int -> V.Vector Int -> HashMap Int (V.Vector Int)
mapping n v = do
    runST $ do
        mp <- foldM step HMap.empty (V.zip [1..n] v)
        traverse freezeGV mp
    where
        step mp (i, x) = do
            HMap.alterF (f i) x mp
        f i m = case m of
            Nothing -> do
                gv <- newGV
                pushGV gv i
