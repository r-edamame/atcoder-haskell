...
main = do
    result <- flip fmap BS8.getContents $ runInput $ do
        a <- readInt
        n <- readInt
        return . show $ solv a n
    putStrLn result

solv :: Int -> Int -> Int
solv a n = runST $ do
    arr <- V.thaw $ V.replicate 1000001 False
    flip runReaderT arr $ go (Seq.singleton @(Int, Int) (1, 0))
    where
        nd = getDigit n * 10
        go :: Seq (Int, Int) -> ReaderT (MV.MVector s Bool) (ST s) Int
        go Seq.Empty = return (-1)
        go ((i, cnt) :<| sq)
            | i == n = return cnt
            | otherwise = do
                let !c = debug "cnt" cnt
                sq1 <- ifM (cr i) (return $ sq :|> (debug "rotate" $ rotate10 i, cnt+1)) (return sq)
                sq2 <- ifM (ca i) (return $ sq1 :|> (debug "a*i" $ a*i, cnt+1)) (return sq1)
                go sq2
        cr i
            | i < 10 || i`mod`10 == 0 = return False
            | otherwise = do
                let rotated = rotate10 i
                if rotated > min nd 1000000 then return False else do
                    used <- readVr rotated
                    when (not used) $ writeVr rotated True
                    return (not used)
        ca i
            | i*a > min nd 1000000 = return False
            | otherwise = do
                used <- readVr (i*a)
                when (not used) $ writeVr (i*a) True
                return (not used)

rotate10 :: Int -> Int
rotate10 n = if n < 10 then n else n `div` 10 + ((n`mod`10)*d) where
    d = getDigit n

getDigit n = go 1 where
    go d
        | n < d = d `div` 10
        | otherwise = go (d*10)
...
---- util for reader
readVr :: PrimMonad m => Int -> ReaderT (MV.MVector (PrimState m) a) m a
readVr i = do
    v <- Reader.ask
    lift $ MV.read v i
{-# INLINE readVr #-}
writeVr :: PrimMonad m => Int -> a -> ReaderT (MV.MVector (PrimState m) a) m ()
writeVr i x = do
    v <- Reader.ask
    lift $ MV.write v i x
{-# INLINE writeVr #-}
