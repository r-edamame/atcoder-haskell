...
main = do
    result <- flip fmap BS8.getContents $ runInput $ do
        vs <- V.map digitToInt . toCharVector <$> readWord
        return . show $ solv vs
    putStrLn result

solv :: V.Vector Int -> Int
solv v = sum $ V.map (sum . V.map (uncurry (*)) . (`V.zip` ds)) $ vs where
    vs = V.iterateN 3 rotateL v
    ds = V.iterateN 3 (`div`10) 100


rotateL :: V.Vector Int -> V.Vector Int
rotateL v = V.snoc (V.tail v) (V.head v)
