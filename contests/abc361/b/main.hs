...
instance HasField "x" (a, a, a) a where
    getField (x, _, _) = x
instance HasField "y" (a, a, a) a where
    getField (_, y, _) = y
instance HasField "z" (a, a, a) a where
    getField (_, _, z) = z

main :: IO ()
main = runInput $ do
    a <- (,,) <$> readInt <*> readInt <*> readInt
    b <- (,,) <$> readInt <*> readInt <*> readInt
    c <- (,,) <$> readInt <*> readInt <*> readInt
    d <- (,,) <$> readInt <*> readInt <*> readInt
    yesno $ solv a b c d

solv :: (Int, Int, Int) -> (Int, Int, Int) -> (Int, Int, Int) -> (Int, Int, Int) -> Bool
solv a b c d = and @[]
    [ a.x < d.x && c.x < b.x
    , a.y < d.y && c.y < b.y
    , a.z < d.z && c.z < b.z
