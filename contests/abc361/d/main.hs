...
main = runInput $ do
    ...
    t <- V.fromList . BS8.unpack <$> readWord
    liftIO $ print $ solv n s t

solv :: Int -> VC -> VC -> Int
solv n s t = fromMaybe (-1) $ searchFind f snd ([(0, s, n)] :: Seq (Int, (VC, Int))) where
    f :: (Int, VC, Int) -> Either Int [(Int, VC, Int)]
    f (c, vc, sp)
        | V.slice 0 n vc == t = Left c
        | otherwise = Right
            [(c+1, V.fromList $ map (\k -> []), i)
                | i <- [0..n-1], i /= sp-1]
        where
            -- is = fromJust $ V.findIndex (=='.') vc
            sl l r = V.slice l r vc
...
searchM :: (PushPop pp, Ord a, Monad m, Foldable f) => (Elem pp -> m (f (Elem pp))) -> (Elem pp -> a) -> pp -> m (Set a)
searchM f uq = go Set.empty where
    p dup a pp = if Set.member a dup then pp else push a pp
    go dup pp = case pop pp of
        Just (a, pp')
            | Set.member (uq a) dup -> go dup pp'
            | otherwise -> f a >>= go (Set.insert (uq a) dup) . foldr push pp'
...
searchFindM :: (Monad m, PushPop pp, Ord a, Foldable f) => (Elem pp -> m (Either b (f (Elem pp)))) -> (Elem pp -> a) -> pp -> m (Maybe b)
searchFindM f uq ini = flip runContT (return . const Nothing) $ searchM f' uq ini where
    f' a = lift (f a) >>= \case
        Right nx -> return nx
        Left res -> ContT $ \_ -> return (Just res)
searchFind :: (PushPop pp, Ord a, Foldable f) => (Elem pp -> Either b (f (Elem pp))) -> (Elem pp -> a) -> pp -> Maybe b
...
instance Ord a => PushPop (Set a) where
    ...
    pop st
        | Set.null st = Nothing
        | otherwise = Just $ Set.deleteFindMin st
