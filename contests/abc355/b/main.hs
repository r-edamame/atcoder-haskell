...
main = runInput $ do
    ...
    as <- readInts n
    bs <- readInts m
    liftIO . putStrLn . yesno $ solv n m as bs


solv :: Int -> Int -> VI -> VI -> Bool
solv n m as bs = isJust . L.find (\v -> snd (V.head v) == True && v.len > 1) $ V.groupBy (\a b -> snd a == snd b) cb where
    cb = V.modify (VH.sortBy compare) $ (V.zip as $ V.replicate n True) V.++ (V.zip bs $ V.replicate m False)
