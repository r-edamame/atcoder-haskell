...
main = runInput $ do
    ...
    t <- readInt
    as <- readInts1 t
    liftIO $ print $ solv n t as

solv :: Int -> Int -> VI -> Int
solv n t as = runST $ do
    h <- V.thaw $ V.replicate n 0
    v <- V.thaw $ V.replicate n 0
    go 1 h v 0 0 (toList as)
    where
        go :: Int -> MV.MVector s Int -> MV.MVector s Int -> Int -> Int -> [Int] -> ST s Int
        go _ _ _ _ _ [] = return (-1)
        go i h v c1 c2 (a:as')= do
            let x = a`mod`n
            let y = a`div`n
            h.modify x succ
            v.modify y succ
            let c1' = if x == y then succ c1 else c1
            let c2' = if x == n - 1 - y then succ c2 else c2
            ok <- or <$> sequence @[] [(==n) <$> h.at x, (==n) <$> v.at y, return (c1'==n), return (c2'==n)]
            if ok then return i else go (succ i) h v c1' c2' as'
